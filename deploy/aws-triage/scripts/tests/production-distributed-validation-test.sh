...
grep -Fq 'GATEWAY_AI_SAFETY_SIDECAR_URL: http://${GATELM_PRODUCTION_DISTRIBUTED_PII_UPSTREAM_HOST:-10.78.2.50}:8001/internal/ai-safety/v1/detect' "${COMPOSE_PATH}"
grep -Fq 'GATEWAY_AI_SAFETY_SIDECAR_TIMEOUT_MS: "300"' "${COMPOSE_PATH}"
...
grep -Fq 'GATEWAY_AI_SAFETY_OVERLOAD_POLICY: fail_closed' "${COMPOSE_PATH}"
...
grep -Fq 'GATELM_PRODUCTION_DISTRIBUTED_PII_UPSTREAM_HOST=10.78.2.50' "${ENV_PATH}"
...
grep -Fq 'AI_SERVICE_AI_SAFETY_MAX_CONCURRENT: "2"' "${PII_COMPOSE_PATH}"
grep -Fq 'AI_SERVICE_AI_SAFETY_MAX_PENDING: "4"' "${PII_COMPOSE_PATH}"
grep -Fq 'AI_SERVICE_AI_SAFETY_WAIT_TIMEOUT_MS: "50"' "${PII_COMPOSE_PATH}"
...
grep -Fq 'AI_SERVICE_ONNX_INTRA_OP_THREADS: "2"' "${PII_COMPOSE_PATH}"
...
grep -Fq 'production_assert_tcp "PII v0.1.1 (legacy v3.14) Service" "${GATELM_PRODUCTION_DISTRIBUTED_PII_UPSTREAM_HOST}" 8001' "${PREFLIGHT_PATH}"
grep -Fq 'production_assert_http_ready "PII v0.1.1 (legacy v3.14) Service" "http://${GATELM_PRODUCTION_DISTRIBUTED_PII_UPSTREAM_HOST}:8001/readyz"' "${PREFLIGHT_PATH}"
...
grep -Fq 'PiiSecondaryInstance:' "${TEMPLATE_PATH}"
grep -Fq 'PrivateIpAddress: 10.78.2.51' "${TEMPLATE_PATH}"
grep -Fq 'PiiLoadBalancer:' "${TEMPLATE_PATH}"
grep -Fq 'PrivateIPv4Address: 10.78.2.11' "${TEMPLATE_PATH}"
grep -Fq 'PiiTargetGroup:' "${TEMPLATE_PATH}"
...
grep -Fq '"PiiSecondaryInstanceId"' "${CD_TEMPLATE_PATH}"
grep -Fq '"MaxSessionDuration": 10800' "${CD_TEMPLATE_PATH}"
grep -Fq 'deploy_order+=(pii ai data)' "${SEND_DEPLOY_PATH}"
grep -Fq 'deploy_order+=(pii-secondary)' "${SEND_DEPLOY_PATH}"
...
grep -Fq 'pii_secondary_instance_id="${12:-}"' "${SEND_DEPLOY_PATH}"
grep -Fq 'pii_upstream_host="${13:-10.78.2.50}"' "${SEND_DEPLOY_PATH}"
...
grep -Fq 's3api delete-object' "${BOOTSTRAP_PII_PATH}"
grep -Fq -- '--sse AES256' "${BOOTSTRAP_PII_PATH}"
grep -Fq 'GATELM_PRODUCTION_DISTRIBUTED_PII_PRIVATE_IP 10.78.2.51' "${BOOTSTRAP_PII_PATH}"
...
! grep -Fq 'chmod 0555 "${extract_dir}"' "${PREPARE_PII_PATH}"
grep -Fq 'mv -- "${extract_dir}" "${model_dir}"' "${PREPARE_PII_PATH}"
grep -Fq 'chmod 0555 "${model_dir}"' "${PREPARE_PII_PATH}"
pii_model_move_line="$(grep -nF 'mv -- "${extract_dir}" "${model_dir}"' "${PREPARE_PII_PATH}" | cut -d: -f1)"
pii_model_lock_line="$(grep -nF 'chmod 0555 "${model_dir}"' "${PREPARE_PII_PATH}" | cut -d: -f1)"
[[ "${pii_model_move_line}" -lt "${pii_model_lock_line}" ]]
...
grep -Fq 'current_uri="$(perf_trim "${current_uri}")"' "${DEPLOY_ROLE_PATH}"
grep -Fq 'current_uri="$(perf_unquote_env_value "${current_uri}")"' "${DEPLOY_ROLE_PATH}"
pii_uri="$(tr -d '\r' < "${ENV_PATH}" | awk -F= '$1 == "GATELM_PRODUCTION_DISTRIBUTED_PII_ARTIFACT_S3_URI" {print $2}')"
