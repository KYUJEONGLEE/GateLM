...
if (parameters.length !== 6) {
  throw new Error(`Expected six distributed SSM commands, observed ${parameters.length}`);
}
const comments = lines.filter((line) => line.startsWith("GateLM distributed deploy "));
const expectedRoles = ["pii", "ai", "data", "gateway-secondary", "gateway-primary", "edge"];
if (comments.length !== expectedRoles.length || comments.some((line, index) => !line.endsWith(` ${expectedRoles[index]}`))) {
  throw new Error(`Unexpected two-Gateway deploy order: ${comments.join(", ")}`);
}
for (const parameter of parameters) {
  const command = parameter.commands?.[0];
  const match = command?.match(/^printf '%s' '([A-Za-z0-9+/=]+)' \| base64 --decode \| bash$/);
  if (!match) throw new Error("Two-Gateway SSM payload is not POSIX-safe");
  const remoteScript = Buffer.from(match[1], "base64").toString("utf8");
  if (!remoteScript.includes('gateway_upstream_host=10.78.2.10')) {
    throw new Error("Two-Gateway payload did not pin the internal NLB upstream");
  }
}
NODE
