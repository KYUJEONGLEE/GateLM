...
clone_compose --profile "${role}" config --quiet
...
if [[ "${check_dependencies}" == "true" ]]; then
  case "${role}" in
    edge)
      clone_assert_tcp "Gateway public data plane" "${GATELM_PROD_CLONE_GATEWAY_PRIVATE_IP}" 8080
    ...
    rag)
      clone_assert_tcp "PostgreSQL" "${GATELM_PROD_CLONE_DATA_PRIVATE_IP}" 5432
      clone_assert_tcp "Private Gateway" "${GATELM_PROD_CLONE_GATEWAY_PRIVATE_IP}" 8081
      clone_assert_tcp "AI Service" "${GATELM_PROD_CLONE_AI_PRIVATE_IP}" 8001
      ;;
    data)
      clone_assert_tcp "AI Service" "${GATELM_PROD_CLONE_AI_PRIVATE_IP}" 8001
