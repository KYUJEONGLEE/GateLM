...
SELECT json_build_object(
  ...
  'provider_id', coalesce(metadata #>> '{providerAttempt,providerId}', ''),
  'model_id', coalesce(metadata #>> '{providerAttempt,modelId}', ''),
  'requested_model', coalesce(requested_model, ''),
  'model_ref', coalesce(metadata #>> '{modelRef}', ''),
  ...
  'provider_latency_ms', CASE WHEN provider_latency_ms >= 0 THEN provider_latency_ms ELSE NULL END,
  'gateway_internal_latency_ms', greatest(coalesce(latency_ms, 0) - greatest(coalesce(provider_latency_ms, 0), 0), 0),
  'ttft_ms', CASE WHEN ttft_ms >= 0 THEN ttft_ms ELSE NULL END,
  'stream', CASE WHEN stream THEN 1 ELSE 0 END,
  'cache_status', coalesce(cache_status, ''),
  'cache_type', coalesce(cache_type, 'none'),
