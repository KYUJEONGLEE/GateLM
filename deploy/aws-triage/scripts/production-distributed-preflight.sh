...
if [[ "${check_dependencies}" == "true" ]]; then
  case "${role}" in
    ...
    gateway)
      ...
      production_assert_tcp "PII v0.1.1 (legacy v3.14) Service" "${GATELM_PRODUCTION_DISTRIBUTED_PII_UPSTREAM_HOST}" 8001
      production_assert_http_ready "PII v0.1.1 (legacy v3.14) Service" "http://${GATELM_PRODUCTION_DISTRIBUTED_PII_UPSTREAM_HOST}:8001/readyz"
      ;;
    data)
