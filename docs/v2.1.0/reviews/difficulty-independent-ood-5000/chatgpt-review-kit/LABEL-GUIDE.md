...
### 10.5 Independent OOD Dataset 2 5,000건 candidate

[`evaluation/difficulty-independent-ood-5000.v1.candidate.jsonl`](evaluation/difficulty-independent-ood-5000.v1.candidate.jsonl)은 Dataset 1의 train/calibration/evaluation/promotion record를 덧붙이거나 교체하지 않는 별도 candidate다.

- 5,000 records, 1,000 `independent2.*` families, family당 5개 surface variant
- category × provisional difficulty 각 500건, Korean 2,000/English 2,000/mixed 1,000
- 44개 독립 renderer와 prose, question, chat, email, ticket, form, table, symbolic, source-delimited 등 서로 다른 surface 구조
- train 3,000/validation 1,000/test 1,000을 600/200/200 family로 사전 분리
- Dataset 1은 prompt 생성이 끝난 뒤 overlap audit에만 사용하며 shared generator module은 0개
- 모든 label은 post-render provisional annotation이고 `pending + reviewerCount=0`; `trainingEligible=false`

Blind review queue는 `sampleId`, `redactedPrompt`, `language` 외의 label·family·split 정보를 포함하지 않는다. 두 reviewer가 독립적으로 category, difficulty, 네 bucket, semantic label과 instruction/payload boundary를 판정한 뒤 일치 건만 승인 후보로 만들고, 불일치 건은 `needs_adjudication` 절차로 보낸다. 현재 candidate와 provisional label은 human approval 또는 model 성능 evidence가 아니다. Human approval 전에는 파일명이 train/validation/test여도 어느 split도 학습·선택·평가에 사용할 수 없다.
