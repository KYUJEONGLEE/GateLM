...
- 인증 헤더: 2026-07-14부터 dashboard/log/detail/cost/analytics server client는 `X-GateLM-Observability-Token`을 보내며, Gateway의 5개 observability read route는 전용 inbound token middleware로 보호한다. self-host/AWS Compose는 이 토큰을 필수로 두고 Web에만 전달한다. 로컬 개발은 `GATEWAY_OBSERVABILITY_AUTH_REQUIRED=false`로 명시적으로 비활성화할 수 있다.
...
- observability endpoint 인증: 전용 `GATEWAY_OBSERVABILITY_INTERNAL_TOKEN`을 기존 Control Plane token/API Key와 분리했다. production-like 또는 `GATEWAY_OBSERVABILITY_AUTH_REQUIRED=true`에서 누락·약한 토큰은 시작 단계에서 거부되고, Web tenant layout 및 polling BFF도 로그인 사용자의 Tenant membership/project-admin scope를 fail-closed 검증한다. public ingress의 `/api/*` 차단은 추가 방어선으로 계속 권장한다.
