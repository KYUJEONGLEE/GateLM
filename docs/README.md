...
이 검증은 `docs/README.md` first 원칙, `contracts.md` 우선순위, Reference/Draft 문서 승격 방지, Semantic Cache evidence-only guardrail, schema/fixture pairing, fixture validation, forbidden sensitive value shape, Provider/Model enum lock 방지, RuntimeSnapshot lookup key guardrail을 확인한다.
