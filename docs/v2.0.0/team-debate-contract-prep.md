...
- terminal status와 domain outcome을 분리한다.
...
v2.0.0 terminal status는 작게 유지한다.
...
v2.0.0 계약에서는 domain outcome을 기반으로 terminal status를 결정하는 규칙이 필요하다.
...
| Safety/Budget/Auth/Policy가 의도적으로 차단함 | `blocked` |
...
v2.0.0 implementation plan 전에 아래 질문을 먼저 확정한다.

1. `cache_hit`을 terminal status에서 제거하고 `success + cache.outcome=hit`로 볼 것인가?
2. auth 실패를 `blocked`로 볼 것인가, `failed`로 볼 것인가?
...
8. Provider/Model catalog는 Control Plane DB에서 관리하고 RuntimeSnapshot에 포함할 것인가?
9. Budget warning은 사용자 응답에 표시할 것인가, 운영자 화면에만 표시할 것인가?
10. Dashboard freshness와 query budget을 API 계약에 포함할 것인가?

가장 먼저 결정할 항목은 `cache_hit`의 위치다.
...
v2.0.0 구현 계획은 아래 순서로 짜는 것이 좋다.

1. Terminal status/domain outcome 계약 확정
2. RuntimeSnapshot publish/reload/provenance 계약 확정
3. Budget scope 계약 확정
4. Actual Provider 1종 + 모델 2개 + Mock fallback 계약 확정
5. Request Detail/Dashboard outcome 표시 계약 확정
6. Streaming thin slice 범위 확정
7. k6 baseline/query profile 기준 확정
8. Semantic Cache evidence track 분리 확정
...
v2.0.0은 terminal status를 작게 유지하고 domain outcome으로 실행 경로를 설명하면서, RuntimeSnapshot, budgetScope, Actual Provider + Mock fallback, Dashboard query profile을 계약으로 승격하는 단계다.
