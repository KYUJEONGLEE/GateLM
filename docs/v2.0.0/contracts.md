...
RuntimeSnapshot `policies.routing.candidateStatuses`는 provider/model 후보의 low-cardinality 상태 입력으로만 사용한다. 허용 값은 `available | degraded | unavailable`이며 raw prompt, raw provider error, credential, header, secret은 포함하지 않는다.
