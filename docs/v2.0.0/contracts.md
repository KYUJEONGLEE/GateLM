...
RuntimeSnapshot `policies.routing.candidateStatuses`는 provider/model 후보의 low-cardinality 상태 입력으로만 사용한다. 이 값은 Policy Editor에서 관리자가 편집하는 RuntimeConfig 필드가 아니며, Gateway 실행 뷰 또는 운영 overlay 입력으로만 취급한다. 허용 값은 `available | degraded | unavailable`이며 raw prompt, raw provider error, credential, header, secret은 포함하지 않는다.
