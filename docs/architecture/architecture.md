...
13. Gateway가 Exact Cache key를 생성
14. Exact Cache hit이면 Provider 호출 없이 응답 반환
15. Exact Cache miss이면 Semantic Cache 후보를 조회
16. Semantic Cache hit이면 정책에 따라 응답 반환
17. Cache miss이면 Model Routing 수행
...
기본 정책은 전체 대화 기록을 매번 보내지 않는다. MVP는 직계 부모 기반 Reply-to Context를 우선한다.
...
MVP에서는 기본 Semantic Cache만 구현한다. Vector DB 같은 신규 인프라는 이 문서에서 확정하지 않는다. 저장 방식과 schema는 `contracts/db_schema.md`에서 확정한 뒤 구현한다.
