...
## 1.5 AI Insights 분석 경계

Overview Dashboard의 AI Insights는 기본적으로 mock mode로 동작한다.
실제 AI 분석을 켜려면 서버 환경변수에 provider key를 설정하고 `AI_INSIGHTS_ENABLED=true`를 사용한다.

```env
AI_INSIGHTS_ENABLED=true
AI_INSIGHTS_PROVIDER=openai
AI_INSIGHTS_BASE_URL=https://api.openai.com/v1
AI_INSIGHTS_MODEL=gpt-5.5
AI_INSIGHTS_API_KEY=replace-with-server-side-key
AI_INSIGHTS_TIMEOUT_MS=8000
```

규칙:

- Provider API key는 서버 환경변수에서만 읽는다.
- `NEXT_PUBLIC_`, `VITE_`, `REACT_APP_` 같은 public env 이름으로 provider key를 노출하지 않는다.
- AI Insights provider 입력은 Dashboard aggregate와 최근 요청의 sanitized summary만 사용한다.
- raw prompt, raw response, API Key, App Token, Provider Key, Authorization header, provider raw error body는 AI prompt, 로그, UI에 포함하지 않는다.
- API key가 없거나 provider 호출이 실패하면 mock/fallback insight를 반환하며 Dashboard 전체를 깨지 않는다.
...
| 지표 | API Field | 설명 |
|---|---|---|
| 차단 요청 수 | `blockedRequests` | 예산, Quota, 보안 정책 차단 |
| Rate Limit 요청 수 | `rateLimitedRequests` | Rate Limit 정책으로 차단된 요청 |
...
| `rate_limited` | 사용량 제한 | Yes | No | No | No | No |
...
- `rate_limited`는 Rate Limit 정책이 의도대로 작동한 결과다. 실패나 에러로 보지 않는다.
...
```text
success, cache_hit, partial_success, error, blocked, rate_limited, cancelled
```
...
```text
errorEligibleRequests = totalRequests - blockedRequests - rateLimitedRequests - cancelledRequests
...
rateLimitedRequests: status = 'rate_limited'
...
- `rate_limited`는 정책 성공이므로 기본 에러율 분모에서 제외한다.
...
## 6.2 `rateLimitedRequests`

```text
rateLimitedRequests = countDistinct(requestId where status = 'rate_limited')
```

Rate Limit 정책으로 Provider 호출 전에 차단된 요청 수다.

## 6.3 `cancelledRequests`
...
| `rateLimitedRequests` | `countDistinctIf(status = 'rate_limited')` | count | `llm_invocations.status` | rate_limited |
...
| `errorRate` | `failedRequests / (totalRequests - blockedRequests - rateLimitedRequests - cancelledRequests)` | ratio | derived | error eligible |
...
rateLimitRate = if totalRequests > 0 then rateLimitedRequests / totalRequests else null
```
...
- `rate_limited`가 failedRequests와 errorRate denominator에서 제외된다.
...
- [ ] `rateLimitedRequests`가 별도 집계된다.
- [ ] `errorRate` 분모에서 `blocked`, `rate_limited`, `cancelled`가 제외된다.
