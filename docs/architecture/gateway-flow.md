...
13. Gateway가 Exact Cache를 조회한다
14. Exact Cache miss이면 Semantic Cache를 조회한다
15. Cache miss이면 Model Routing을 수행한다
16. Gateway가 Provider credential을 secret reference 기준으로 조회한다
17. Gateway가 Provider Adapter로 request를 변환한다
18. Gateway가 Provider를 호출하거나 SSE stream을 relay한다
19. Gateway가 Provider response를 OpenAI-compatible response로 변환한다
20. Gateway가 token / cost / latency / cache / routing / masking metadata를 계산한다
21. Cache 저장 조건을 만족하면 cache entry를 저장한다
22. Gateway가 Client에게 응답을 반환한다
23. Gateway가 Redpanda에 usage/log event를 발행한다
24. Worker가 event를 소비해 ClickHouse / PostgreSQL / S3에 저장한다
25. Dashboard와 Request Log API가 저장된 분석 데이터를 조회한다
```
...
MVP에서는 전체 대화 기록을 매번 보내지 않고 **Reply-to Context**를 우선 적용한다.
...
3. exact cache key 생성
4. Redis에서 Exact Cache 조회
5. hit이면 Provider 호출 없이 응답 생성
6. miss이면 Semantic Cache 조회
7. Semantic Cache hit이면 정책에 따라 응답 생성
8. 모든 cache miss이면 Model Routing으로 진행
```
...
7. 마지막 chunk에 request_id, cache_status, routed_provider, routed_model metadata를 포함한다
8. data: [DONE] 전송
...
신규 cache는 cache key 구성, security policy hash, TTL, invalidation 기준을 문서화해야 한다.
...
-> redacted prompt 기준 Cache Key 생성
-> Cache 조회
-> Routing
-> Provider 호출
...
- `resident_registration_number`는 MVP 기본 정책에서 Provider 호출 전 block한다. `api_key`, `authorization_header`, `jwt`는 기본 redaction이며 tenant policy로 block 강화가 가능하다.
