...
The delivery archive is kept at the Git-ignored runtime path `apps/ai-service/.cache/bundles/tenant-chat-pii-model-bundle-20260715.zip`. Runtime artifacts are imported into the Git-ignored `apps/ai-service/.cache/onnx` directory by a manifest allowlist; the archive is not blindly expanded into the repository.
...
-> Tenant safety evaluator, once per message
-> local P0 rules
-> immediate stop when a mandatory/local rule blocks
-> local ai-service sidecar for remaining text
-> cheap rule scan and ML candidate gate
-> OpenAI ONNX, then quantized KoELECTRA ONNX when ML is needed
...
The current Tenant evaluator calls the sidecar once for each message in the request. This branch does not claim a multi-message inference batch implementation. Requests with many history messages can therefore accumulate sidecar overhead, although ML is skipped when cheap rules fully cover the candidate or no ML candidate exists.
...
- Model adapters default to sequential execution because concurrent execution was not faster on the measured CPU. Parallel execution remains opt-in.
