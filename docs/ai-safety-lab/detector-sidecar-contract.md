...
ì´ ë¬¸ì„œëŠ” AI Safety Labì˜ local detector sidecar draft contractë‹¤.

ì´ ê³„ì•½ì€ Lab draftì´ë©° ê¸°ì¡´ v2 Gateway ê³„ì•½ì„ overrideí•˜ì§€ ì•ŠëŠ”ë‹¤. ì œí’ˆ API, DB, Event, Metrics ê³„ì•½ìœ¼ë¡œ ìŠ¹ê²©í•˜ë ¤ë©´ ë³„ë„ ê³„ì•½ ë³€ê²½ì´ í•„ìš”í•˜ë‹¤.
...
ì´ endpointëŠ” Gatewayì™€ ê°™ì€ trusted runtime boundary ì•ˆì˜ CPU-only local process/container/podì—ì„œ í˜¸ì¶œí•˜ëŠ” í›„ë³´ ê²½ë¡œë‹¤. Hosted inference APIë¡œ raw promptë¥¼ ë³´ë‚´ì§€ ì•ŠëŠ”ë‹¤.

## 3. Request Semantics

Sidecar requestëŠ” redaction ê³„ì‚°ì„ ìœ„í•´ transient prompt textë¥¼ ë°›ì„ ìˆ˜ ìˆë‹¤. ì´ ê°’ì€ ë¯¼ê° ì…ë ¥ìœ¼ë¡œ ì·¨ê¸‰í•˜ë©° ì €ì¥, log, error response, fixture, reportì— ë³µì‚¬í•˜ì§€ ì•ŠëŠ”ë‹¤.

Top-level request í›„ë³´:
...
`promptText` exampleì€ synthetic placeholderë§Œ ì‚¬ìš©í•œë‹¤. ì‹¤ì œ ê³ ê° ë¬¸ì¥, ì‹¤ì œ ì´ë©”ì¼, ì‹¤ì œ ì „í™”ë²ˆí˜¸, ì‹¤ì œ token, ì‹¤ì œ credentialì„ ë¬¸ì„œë‚˜ fixtureì— ë„£ì§€ ì•ŠëŠ”ë‹¤.
...
`mode=shadow`ëŠ” sanitized observationê³¼ log-safe redactionë§Œ ì œê³µí•˜ë©° Providerì— ì „ë‹¬í•  promptì™€ ìµœì¢… actionì„ ë³€ê²½í•˜ì§€ ì•ŠëŠ”ë‹¤. `mode=enforce`ì—ì„œë§Œ Gatewayê°€ sidecarì˜ redaction/blockì„ ì‹¤í–‰ ê²°ê³¼ì— ë°˜ì˜í•œë‹¤. `detectorPolicies`ê°€ ìˆìœ¼ë©´ ê°™ì€ detector typeì˜ sidecar ê¸°ë³¸ actionë³´ë‹¤ ìš°ì„ í•˜ë©° Tenant Chat RuntimeSnapshotì˜ `allow|redact|block`ì„ ë³´ì¡´í•œë‹¤.
`GATEWAY_AI_SAFETY_OVERLOAD_POLICY=fail_closed`ëŠ” ê²€ì¦ëœ retryable `503 sidecar_unavailable`ì˜ ìµœì¢… ë™ì‘ì„ ë°”ê¾¸ë¯€ë¡œ `mode=enforce`ì—ì„œë§Œ í—ˆìš©í•œë‹¤. `shadow + fail_closed` ì¡°í•©ì€ ì‹œì‘ ì‹œ ê±°ë¶€í•œë‹¤.
...
SidecarëŠ” `redactedPrompt`ê¹Œì§€ ë§Œë“¤ì–´ ë°˜í™˜í•œë‹¤. GatewayëŠ” sidecarê°€ ì¤€ offsetì„ ì¬í•´ì„í•˜ì§€ ì•ŠëŠ”ë‹¤.

Success response í›„ë³´:
