...
11. Direct sidecar 응답은 `executionSummary`의 `executionMode`, `modelInvocationCount`, `acceptedModelDetectionCount`를 모두 검증한다. 누락·추가 필드, 잘못된 mode/count 조합은 `invalid_response`다.
12. 모델 활성 지연시간은 `executionMode=hybrid`이고 `modelInvocationCount>=1`인 성공 응답만으로 계산한다. Gateway target은 이 정보가 별도 telemetry로 제공되지 않는 한 `unobserved`다.
...
## 모델 실행 요약
| Runtime | rules-only | hybrid | unobserved | modelActiveRequestCount | modelInvocationCount | acceptedModelDetectionCount | p50ModelActiveSidecarLatencyMs | p95ModelActiveSidecarLatencyMs |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
  ...
  "runtimeResults": [
    {
      ...
      "p50ModelActiveSidecarLatencyMs": null,
      "p95ModelActiveSidecarLatencyMs": null,
      "executionModeCounts": {"rules_only": 100},
