...
| 검증 대상 코드 | `15a41ab4` 이후 2단계 평가 구현 포함 |
| 검증일 | 2026-07-16 KST |
| 대상 경로 | Tenant Chat 저장 전 sanitization과 AI Service ONNX PII 탐지 |
| 데이터 범위 | 합성 문장과 aggregate 결과만 사용 |
| 원문 저장 | 없음 |
| 현재 결정 | 103건 screening 기준 `rules + OpenAI(전화번호·secret만 허용)`를 3단계 성능 후보로 채택. KoELECTRA는 hot path에서 제외 |
...
4. KoELECTRA BIO/BIOES span 병합 결함은 수정됐고 AI Service 전체 회귀 258건이 통과했다.
...
10. 3단계에서는 이 제한된 OpenAI 단독 후보만 지연·메모리·preload·timeout fallback 대상으로 검증한다.
...
- master report v2는 adapter별 invocation과 contribution을 제공하지만 기존 latency benchmark에는 아직 같은 model-active 증거가 없다.
- 선택된 OpenAI 허용목록을 실제 서비스 설정과 self-host smoke가 동일하게 적용하는지는 3단계에서 고정해야 한다.
...
4. OpenAI 모델은 전화번호와 secret만 허용하고, 3단계 성능·fallback gate 통과 전에는 production 기본값이라고 선언하지 않는다.
...
1단계 adapter correctness와 2단계 모델별 기여도 분리는 완료됐다. 현재 선택은 다음과 같다.

```text
유지: Gateway/AI Service 규칙 backstop
3단계 후보: OpenAI privacy-filter ONNX
ML 허용 유형: phone_number, secret
제외: KoELECTRA additional model
```

선택 후보는 rules-only보다 exact pass가 5건 증가하고 false positive는 증가하지 않았지만, 이 수치는 합성 screening 결과다. 따라서 다음 작업은 모델을 더 붙이거나 학습하는 것이 아니라 model-active 지연·메모리·preload·timeout fallback을 측정해 서비스 경로에 들어갈 수 있는지 판정하는 것이다.
