...
실제 실행 결과와 원본 증거 식별자는 [`gateway-scaleout-technical-challenge-results.md`](./gateway-scaleout-technical-challenge-results.md)에 누적한다.
...
|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|
| 1 | 10 | 10.00 | 0% | 0 | 116.129ms | 179.328ms | 300 | 0 | 301 | 통과 |
| 2 | 10 | 10.03 | 0% | 0 | 117.639ms | 186.331ms | 150 | 151 | 302 | 통과 |
| 1 | 50 | 50.03 | 0% | 0 | 113.205ms | 156.462ms | 1,501 | 0 | 1,502 | 통과 |
| 2 | 50 | 50.03 | 0% | 0 | 114.182ms | 144.026ms | 750 | 751 | 1,502 | 통과 |
| 1 | 100 | 100.00 | 0% | 0 | 116.010ms | 154.873ms | 3,000 | 0 | 3,001 | 통과 |
| 2 | 100 | 100.00 | 0% | 0 | 113.184ms | 155.704ms | 1,500 | 1,500 | 3,001 | 통과 |
| 1 | 150 | 150.03 | 0% | 0 | 137.834ms | 162.600ms | 4,501 | 0 | 4,502 | CPU 포화·지연 상승 |
| 2 | 150 | 150.03 | 0% | 0 | 115.404ms | 156.145ms | 2,251 | 2,250 | 4,502 | 통과·p95 개선 |

Mock 호출 수에는 각 k6 실행의 사전 점검 1회가 포함된다. 모든 실행은 30초이므로 최대 처리량이나 지속 가능 SLO가 아니다.

### 분산 정확성 비교

| 시나리오 | 기대값 | 관측값 | 원본 증거 | 결과 | 해결 여부 |
|---|---|---|---|---|---|
| 동일 키 동시 요청 | Provider 호출 1회 | ProviderAttempt·Mock 각 1회 | `e9b7c31e…`, `de8de812…` | 정상 동시 요청에서 중복 미재현 | 변경 없음 |
| 동일 주체 Rate Limit | 전체 허용 수가 제한 이하 | 제한 4, 동시 10건 중 4 허용·6 거절 | `6891301e…`, `10adf649…` | 전역 일관성 통과 | 변경 없음 |
| Assistant 저장 | 1행 | full Chat API persistence 경로 미실행, 0행 | `8e942c7b…` | 미측정 | 의도적으로 미해결 |
| 비용 정산 | 1회 | 확정 비용 1회, attempt count 1 | `de8de812…` | 정상 동시 요청에서 중복 미재현 | 변경 없음 |
| 타 Gateway SSE 재연결 | 진행 중 stream attach | 두 번째 Gateway가 HTTP 503 반환 | `e9b7c31e…` | 실패 재현 | 의도적으로 미해결 |
| 같은 Gateway 등록 전 attach | 진행 중 stream attach | 20ms 동시 요청이 HTTP 503 반환 | `fac55ce7…` | 경쟁 구간 재현 | 의도적으로 미해결 |
| 완료 후 같은 Gateway replay | Provider 재호출 없이 terminal replay | replay HTTP 200, Provider 1회 | `e35f64ce…`, `55ab7aff…` | 대조군 통과 | 변경 없음 |
| Gateway 종료 후 재시도 | 중복 호출·최종 상태 확인 | 실행하지 않음 | 없음 | 미측정 | 의도적으로 미해결 |
