...
- routing decision은 cache lookup과 usage reservation 전에 고정한다. 선택된 `simple|complex`는 content-free `routingDifficulty`로 reservation과 terminal event에 보존하고, 분석 시 route tier 또는 provider/model에서 역추론하지 않는다. exact-cache fingerprint는 snapshot digest와 선택 `modelRef`를 포함하므로 같은 prompt라도 다른 routing target의 response를 재사용하지 않는다.
