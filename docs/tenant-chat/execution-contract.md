...
- `policies.safety.detectorSet`은 detector별 `allow|redact|block` 실행 규칙이며 mandatory secret detector는 `allow`를 거부한다. safety는 routing/cache/Provider보다 먼저 실행하고 redacted input만 다음 단계에 전달한다. `block`은 현재 turn의 마지막 user message에서 탐지됐을 때 요청을 거부한다. 이전 대화 또는 system/RAG context에서 탐지된 값은 현재 정책의 안전 placeholder로 마스킹하고 요청을 계속하며 원문을 Provider에 전달하지 않는다.
