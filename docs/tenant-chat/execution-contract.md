...
- routing decision은 cache lookup과 usage reservation 전에 고정한다. exact-cache fingerprint는 cache policy, 전체 safety policy, routing policy/decision hash와 선택 `modelRef`를 포함하므로 같은 prompt라도 다른 cache/safety/routing target의 response를 재사용하지 않는다. quota/budget만 바뀐 새 Snapshot은 기존 exact cache entry와 호환되며, cache hit 뒤에만 usage reservation을 건너뛴다.
