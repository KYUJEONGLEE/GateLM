...
- routing decision은 cache lookup과 usage reservation 전에 고정한다. 선택된 `simple|complex`는 content-free `routingDifficulty`로 reservation과 terminal event에 보존하고, 분석 시 route tier 또는 provider/model에서 역추론하지 않는다. Gateway는 cache lookup 전에 admission의 immutable server-owned safety summary를 읽으며 safety enabled에서는 완전한 `maskingAction=none`만 cache-eligible이다. `redacted|blocked` 또는 summary 부재는 lookup/write를 모두 우회하고 reservation과 terminal에 `cacheOutcome=off`를 보존한다. exact-cache fingerprint는 cache policy, 전체 safety policy, routing policy/decision hash와 선택 `modelRef`를 포함하므로 같은 prompt라도 다른 cache/safety/routing target의 response를 재사용하지 않는다. quota/budget만 바뀐 새 Snapshot은 기존 exact cache entry와 호환되며, cache hit 뒤에만 usage reservation을 건너뛴다.
...
- 관리자 최초 발행 기본값은 request rate `60/60s`, user concurrency `2`와 admission TTL `30s`, 월 token limit `1,000,000` 및 `80/90/100`, 월 budget `1,000,000,000 microUSD` 및 `80/90/100`, timezone `Asia/Seoul`, provider token rate `120,000/60s`, Exact Cache `exact/enabled`(TTL 300초, 사용자당 최대 100개), email redact/API-key block safety, streaming `120s`와 required final이다. 직원 주간 token quota는 기본 비활성이고 Snapshot에 명시된 signed employee actor에만 적용한다.
...
활성 employee 주간 `limitTokens`는 같은 Snapshot의 `defaultMonthlyTokenLimit`을 초과할 수 없다. Control Plane은 직원 한도를 저장하거나 공통 월간 한도를 낮춰 발행할 때 이 관계를 검증하고, Gateway도 상충하는 Snapshot을 거부한다.
