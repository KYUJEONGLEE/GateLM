...
이 문서는 프로바이더/모델별 토큰 가격표와 요청 비용 계산을 구현하기 전의 계획서다.
...
| 예산 귀속 | 비용/쿼터/대시보드 귀속은 `budgetScopeType/budgetScopeId/resolvedBy`다. | 요청 비용과 원장 항목은 `tenantId/projectId/applicationId/budgetScopeType/budgetScopeId/resolvedBy`를 함께 사용한다. |
| 민감 정보 | 원본 프롬프트, 원본 응답, 키, 토큰, Authorization 헤더, 프로바이더 원본 오류 본문 저장 금지. | 가격/비용 메타데이터에는 토큰 수, 안전한 모델 식별자, 가격표 버전만 저장한다. |
| 요청 상세 | `usageSummary.예상비용MicroUsd`가 존재한다. | 첫 단계에서는 기존 `cost_micro_usd`를 `예상비용MicroUsd` 읽기 모델로 일관되게 매핑한다. |
...
| 비용 정책 | `docs/policies/cost-policy.md`는 versioned price catalog, USD canonical currency, micro USD 정수 계산, 예상/실제 비용 구분 원칙을 이미 제시한다. |
...
| `source` | 가격 출처명 또는 internal demo source |
...
| `cost_micro_usd` | Gateway가 계산한 요청 비용 |
...
다만 요청 로그 저장 후 `cost_micro_usd > 0`일 때 기존 `budget_ledger_entries` 갱신/삽입 흐름을 유지한다.
...
- 정확 캐시 적중은 프로바이더 비용 0원과 절감 비용을 분리한다.
...
| `예상비용MicroUsd`와 실제 비용을 분리할까? | 스키마에는 `예상비용MicroUsd`가 있으나 DB 컬럼은 `cost_micro_usd`다. 첫 단계에서는 프로바이더 사용량 기반 계산값을 저장하고, 이름 정리는 계약 PR에서 다룬다. |
...
- 프로바이더 응답의 사용량 토큰으로 실제 요청 비용을 계산한다.
...
- 프로바이더 호출 전 예상 토큰/비용로 예산 초과 여부를 판단한다.
...
- 프로바이더 호출 후 실제 비용은 요청 로그와 원장을 갱신한다.
- 프론트 파일은 수정하지 않는다.
...
- PR-4의 사전 추정과 PR-2의 실제 비용 저장을 하나의 운영 흐름으로 검증한다.
...
- 현재 합계와 예상 요청 비용의 합이 한도를 넘으면 프로바이더를 호출하지 않는다.
...
- OpenAI 저비용 모델 요청 비용이 계산된다.
- Gemini 균형형 모델 요청 비용이 계산된다.
- Claude 고품질 모델 요청 비용이 계산된다.
- 프로젝트 월별 비용 집계.
- 팀 예산 범위 비용 집계.
- 경고 임계치 요청.
- 하드 차단 요청.
...
| PR-2 | Gateway 실제 비용 | 프로바이더 사용량 기반 비용 계산과 요청 로그 저장을 먼저 안정화한다. |
