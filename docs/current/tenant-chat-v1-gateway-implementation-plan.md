...
| Contract | PR #293 `docs/tenant-chat`, contract ID `tenant-chat/v1` |
| Contract head reviewed | `3448f093` (implementation uses the final head merged to `dev`) |
...
다음 8개 record를 PR #293 DDL과 같은 의미로 additive migration에 추가한다.
...
PR #293 구현 전 최종 확인 대상으로 다음 차이를 추적한다.
