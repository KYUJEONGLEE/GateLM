...
- 최소 15자, 최대 256자
- 필수 대문자·소문자·숫자·특수문자 조합 규칙 없음
- 공백을 포함한 긴 passphrase 허용
- 한 문자 반복, 짧은 패턴 반복, 제품이 차단한 흔한 비밀번호 거부
- 거부 응답 code: `WEAK_PASSWORD`
...
- 요청한 브라우저에만 새 `full` session cookie를 발급한다.
...
- authenticated change 후 current session rotation과 other-session revocation test
