...
| Applies to | Project/Application Dashboard reader, Tenant Chat shadow rollup writer, Request Log TTFT |
| Does not apply to | Billing ledger or quota enforcement period semantics |
| Last reviewed | 2026-07-14 |

## 1. Goal

Dashboard 조회 비용이 raw invocation row 수에 비례해 계속 증가하지 않도록 PostgreSQL에 mergeable `hour`, `day`, `month` 집계를 둔다. 동시에 streaming 응답 시작 속도인 TTFT와 전체 응답 시간을 서로 다른 지표로 제공한다.
...
- `hour`, `day`, `month` bucket은 UTC 경계로 정렬한다.
...
- 집계값에 blind increment하지 않고 해당 bucket을 source에서 전부 다시 계산해 replacement한다.
- hour 완료 후 parent day, day 완료 후 parent month를 dirty로 표시한다.
...
- background writer는 feature flag로 점진 활성화한다.
- schema migration을 먼저 적용하고 shadow parity 검증 후 긴 범위 reader를 전환한다.
