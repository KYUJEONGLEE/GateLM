...
| DOC-036 | PII process-local admission은 `dev`에 병합됐지만 bounded wait와 fail-closed의 목표 운영 환경 evidence가 없다 | [`../testing/pii-inference-concurrency-v0.1.1-20260731.md`](../testing/pii-inference-concurrency-v0.1.1-20260731.md)는 Windows 8-CPU direct 측정과 즉시 거절 방식의 in-process ASGI HTTP 측정이다. 이번 변경은 active 1, pending 4, wait 50ms와 AWS production-distributed fail-closed/300ms timeout을 추가하지만 4-vCPU Linux, Uvicorn/network, multi-worker, client disconnect 및 Gateway E2E를 증명하지 않는다 | 기본 active 1을 유지하고 목표 환경에서 queued success/503 비율, p95/p99, cancellation, `CHAT_RUNTIME_UNAVAILABLE`, public `/v1` 안전 오류, multi-worker 합산 경계를 반복 검증하기 전에는 production SLA나 성능 개선 수치로 선언하지 않음 |
