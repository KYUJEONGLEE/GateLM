...
| DOC-018 | `difficulty-feature-vector.v2`의 optional startup shadow runtime은 구현됐지만 request-level 승격 evidence가 없다 | [`../routing/difficulty-e5-encoder.md`](../routing/difficulty-e5-encoder.md)의 pinned QInt8 encoder/PCA 64D, fixed 4-head/12D와 calibration-only candidate selection으로 고정한 118D decision head·calibrator가 exact component hash를 가진 artifact로 생성됐다. Linux amd64 optional image/runtime lock, 시작 시 hash 검증, failure isolation과 English/Korean/right-truncation 3건의 Python 0.21.2 ↔ Go/Rust 0.22.0 pooled 384D numeric parity gate가 추가됐다. 다만 기존 Holdout의 `general` per-category safety gate 실패, request-level latency·memory evidence, shadow 결과 소비 위치와 active runtime contract는 여전히 pending임 | 현재 bundle을 제품 request path나 routing decision으로 승격하지 않음. 새 promotion artifact와 untouched Holdout이 모든 gate를 통과한 뒤 request-level opt-in shadow 위치·bounded overhead·rollback을 먼저 승인하고, exact artifact version·bundle hash·threshold policy에 대한 owner의 명시적 promotion을 요구하며 자동 승격하지 않음 |
