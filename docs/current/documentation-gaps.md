...
| DOC-025 | Tenant Chat PII v0.1.1의 production 배포는 완료됐지만 model-active PII E2E evidence가 없다 | `7dff2389`가 production target을 v0.1.1(legacy v3.14)로 전환했고, main SHA `fbe6b766...`의 [deploy run `29846604941`](https://github.com/KYUJEONGLEE/GateLM/actions/runs/29846604941)이 private S3 artifact 설치·검증, PII role health, public auth와 Tenant Chat smoke까지 성공했다. 다만 smoke는 v0.1.1 마스킹 결과, Shadow/Canary와 현재 online 상태를 증명하지 않는다 | 완료된 배포를 package-ready 또는 미승격으로 되돌리지 않는다. production-grade DLP라고 주장하기 전 model-active Gateway PII E2E, realistic concurrency/timeout/fallback와 Shadow/Canary evidence를 남김 |
