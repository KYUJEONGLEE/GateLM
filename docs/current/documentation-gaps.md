...
| DOC-025 | Tenant Chat PII v0.1.1의 production 배포와 local·기존 AWS PII host 격리 동일 모델 Shadow E2E는 완료됐지만 production model-active E2E는 없다 | main SHA `fbe6b766...`의 [deploy run `29846604941`](https://github.com/KYUJEONGLEE/GateLM/actions/runs/29846604941)이 private S3 artifact 설치·검증과 PII role health를 통과했다. [AWS test-tenant Shadow E2E](../testing/pii-shadow-v0.1.1-test-tenant-e2e-20260802.md)는 기존 4-vCPU Linux PII host의 network-none 컨테이너에서 allowlisted 합성 요청 1,000건 중 52건 비교, model-active 21건, agreement 100%, 오류 0건, control tenant 제외와 live request pause를 확인했다. 다만 사용자 승인으로 야간 window를 예외 처리했고 production traffic, 배포된 서비스 프로세스, authenticated 전체 Chat API와 multi-worker는 증명하지 않는다 | 완료된 배포와 격리 배관 검증을 production-grade DLP로 확대 해석하지 않는다. 정상 야간 window에서 authenticated Gateway PII E2E, realistic concurrency/timeout/fallback, 장시간 multi-replica Shadow/Canary와 rollback evidence를 남김 |
...
| DOC-036 | PII process-local admission은 `dev`에 병합됐지만 bounded wait와 fail-closed의 목표 운영 환경 E2E evidence가 없다 | 기존 [동시성 측정](../testing/pii-inference-concurrency-v0.1.1-20260731.md)에 이어 [고정 4 CPU 예산 측정](../testing/pii-thread-budget-matrix-v0.1.1-20260731.md)이 Windows 4-CPU affinity에서 1×4, 2×2, 4×1과 active 1/2 bounded ASGI HTTP를 비교했다. 2026-08-01 실제 4-vCPU Linux PII host의 aggregate-only direct 재검증에서도 2×2는 오류·불일치 0건과 낮은 p99를 보였지만 Uvicorn/network, multi-worker, client disconnect 및 Gateway E2E는 증명하지 않는다 | 고정 4-vCPU AWS profile은 active 2/intra-op 2로 제한하고, queued success/503 비율, p95/p99, cancellation, `CHAT_RUNTIME_UNAVAILABLE`, public `/v1` 안전 오류, multi-worker 합산 경계를 반복 검증하기 전에는 production SLA나 end-to-end 성능 개선 수치로 선언하지 않음 |
| DOC-037 | tenant별 사용자 정의 PII 룰의 생성·배포·폐기 계약이 없다 | built-in 룰은 Python/Gateway 중앙 registry로 모였고 code-level detector 조합 지점은 존재하지만, Admin API·DB·RuntimeSnapshot·UI·감사·롤백 의미는 정의되지 않았다. 검증되지 않은 정규식을 그대로 받으면 ReDoS, 과도한 마스킹, snapshot 불일치 위험이 있다 | `PII-RULE-00`에서 허용 detector/action, RE2 호환 문법, pattern·rule 개수/길이 상한, compile fail-closed, version/digest, tenant 격리, audit, staged rollout과 rollback을 승인한 뒤 별도 구현 PR로 연결 |
| DOC-038 | PII Offline Shadow의 durable aggregate 저장·조회 계약이 없다 | 현재 구현은 process-local encrypted buffer에서 비교한 뒤 raw·개별 결과 없이 bounded count와 latency percentile을 sanitized structured log로만 출력한다. AI Service에 ClickHouse credential/writer가 없고 기존 analytics table도 이 aggregate의 lifecycle·idempotency·retention을 정의하지 않는다 | `PII-SHADOW-STORE-00`에서 aggregate schema, producer identity, idempotency, retention, tenant 비식별성, ClickHouse 장애와 drop 정책, read 권한을 승인하기 전에는 DB writer나 UI를 연결하지 않음 |
