...
| DOC-018 | `difficulty-feature-vector.v2`의 opt-in request shadow는 구현됐지만 현재 runtime에서는 비활성이다 | Single-request v2 encoder/PCA manifest와 v3 118D artifact의 historical Linux amd64 native replay는 Python/Go label `100/100`, score tolerance, rule routing/modelRef 불변과 failure isolation을 확인했다. 그러나 v3 promotion Holdout accuracy는 `0.70`, threshold-only v4의 별도 untouched Holdout accuracy는 `0.56`이었다. Routing owner가 승인한 v3 artifact/bundle/threshold와 메모리·rollback guardrail은 historical decision boundary의 exact pair에만 적용된다. Current boundary가 달라 [`../testing/difficulty-live-shadow-boundary-supersession.json`](../testing/difficulty-live-shadow-boundary-supersession.json)이 encoder 생성 전 compatibility failure를 고정한다 | v3/v4를 product routing으로 승격하지 않고 current boundary에서는 allowlist가 있어도 shadow를 `unavailable`로 유지함. 새 exact-boundary artifact가 새 Holdout gate를 통과하고 owner가 artifact, bundle, threshold, memory와 rollback을 다시 승인하기 전에는 live shadow를 활성화하지 않음 |
| DOC-020 | Canonical difficulty decision boundary가 semantic-empty simple sentinel과 combined hard evidence `8+`로 갱신되어 기존 single-request v3/v4 artifact의 학습 membership과 달라졌다 | Owner-approved 2,500개 실제 Go 재감사에서 semantic-empty mismatch는 `100 -> 0`, hard/simple 충돌은 `129 -> 0`이 됐고 현재 model path는 train `1,405`, calibration `475`, legacy holdout `477`이다. [`../testing/difficulty-decision-boundary-audit-2026-07-15.md`](../testing/difficulty-decision-boundary-audit-2026-07-15.md). Generated v3 identity는 historical boundary를 pin하고 current Gateway는 encoder 생성 전에 mismatch를 거부한다 | 기존 artifact version을 덮어쓰거나 새 boundary의 promotion evidence로 사용하지 않는다. 신규 데이터 학습 전에 decision-boundary-aware training policy와 새 artifact identity로 재학습하고, 새 owner approval 전까지 optional shadow를 비활성으로 유지한다 |
