...
| DOC-016 | Hybrid `ComplexityScore`의 single-request immutable artifact는 생성됐지만 새 untouched Holdout promotion gate가 없다 | 기존에 선택된 Candidate C 118D architecture를 유지하고 PCA fit, semantic-head/final-head training, calibration, diagnostic evaluation을 `difficulty-e5-single-request-execution.2026-07-15.v1`의 batch size `1`로 다시 실행했다. v3 artifact는 118 weights, bias, Platt coefficient/intercept, `difficulty-threshold-v1 = 0.45`, PCA와 semantic-head hash를 고정한다. 이미 관찰된 Holdout 100건의 diagnostic 결과는 accuracy `0.91`, 전체 `complex -> simple=1`이지만 `general` category 비악화 gate는 계속 실패하며 promotion evidence로 재사용할 수 없다 | 현재 artifact를 product routing으로 승격하지 않음. Artifact 생성 후 수집한 새 untouched Holdout으로 전체/category별 directional gate를 검증한다. Threshold 변경이 필요하면 Holdout이 아니라 사전 고정한 calibration-side evidence로 단일 전역 `difficulty-threshold-v2`를 제안하며 category별 threshold는 만들지 않음 |
...
| DOC-018 | `difficulty-feature-vector.v2`의 opt-in request shadow는 구현됐지만 product routing 승격 evidence가 없다 | Single-request v2 encoder/PCA manifest와 v3 118D artifact를 Linux amd64 optional image/runtime lock에 연결했다. 동일 100건 native replay 3회에서 Python/Go label `100/100`, score 최대 delta `1.71361551770666e-6` 이내, rule routing/modelRef `100/100` 불변, accuracy `0.91`, `complex -> simple=1`을 재현했고 latency·memory, bounded busy queue와 timeout 후 recovery를 측정했다. Global enable과 인증된 tenant/application exact-pair allowlist의 dual opt-in 및 aggregate-only observer 경계도 유지한다. 다만 이 100건은 diagnostic이며 새 untouched Holdout, 기존 `general` per-category regression 해결과 실제 개발 scope live evidence는 없다 | 현재 bundle을 product routing decision으로 승격하지 않음. 제한된 개발 scope의 live 결과는 aggregate disagreement와 category별 baseline-relative 방향만 관찰한다. 새 untouched Holdout이 모든 gate를 통과하고 rollback 기준이 승인된 뒤 exact artifact version·bundle hash·threshold policy에 대한 owner의 명시적 promotion을 요구하며 자동 승격하지 않음 |
