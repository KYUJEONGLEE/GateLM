...
| DOC-036 | PII process-local admission은 `dev`에 병합됐지만 bounded wait와 fail-closed의 목표 운영 환경 evidence가 없다 | 기존 [동시성 측정](../testing/pii-inference-concurrency-v0.1.1-20260731.md)에 이어 [고정 4 CPU 예산 측정](../testing/pii-thread-budget-matrix-v0.1.1-20260731.md)이 Windows 4-CPU affinity에서 1×4, 2×2, 4×1과 active 1/2 bounded ASGI HTTP를 비교했다. 2×2는 재검증 후보지만 HTTP p99가 임시 300ms timeout에 가까우며 4-vCPU Linux, Uvicorn/network, multi-worker, client disconnect 및 Gateway E2E는 증명하지 않는다 | 기본 active 1을 유지하고 목표 환경에서 queued success/503 비율, p95/p99, cancellation, `CHAT_RUNTIME_UNAVAILABLE`, public `/v1` 안전 오류, multi-worker 합산 경계를 반복 검증하기 전에는 production SLA나 성능 개선 수치로 선언하지 않음 |
