...
| DOC-018 | `difficulty-feature-vector.v2`의 opt-in request shadow는 구현됐지만 product routing 승격 gate는 실패했다 | Single-request v2 encoder/PCA manifest와 v3 118D artifact의 기존 Linux amd64 native replay는 Python/Go label `100/100`, score tolerance, rule routing/modelRef 불변과 failure isolation을 확인했다. 이후 새 score-independent 100건 promotion Holdout을 소비한 결과는 accuracy `0.70`, `complex -> simple=0`, category별 directional 비악화 통과였으나 accuracy `>=0.91` gate 실패다. Evidence는 [`../testing/difficulty-promotion-holdout-100-result.json`](../testing/difficulty-promotion-holdout-100-result.json)에 aggregate만 보존한다 | 현재 bundle을 product routing decision으로 승격하지 않음. 개발 exact-pair opt-in도 product decision이 아닌 aggregate disagreement 관찰에만 사용하며, 같은 Holdout으로 재튜닝하지 않음 |
