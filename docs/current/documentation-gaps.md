...
| DOC-025 | Tenant Chat PII v0.1.1의 production 배포와 local 동일 모델 Shadow E2E는 완료됐지만 production model-active E2E는 없다 | main SHA `fbe6b766...`의 [deploy run `29846604941`](https://github.com/KYUJEONGLEE/GateLM/actions/runs/29846604941)이 private S3 artifact 설치·검증과 PII role health를 통과했다. [local Shadow E2E](../testing/pii-shadow-v0.1.1-e2e-20260801.md)는 실제 Gateway sampler·HTTP adapter·기준/후보 ONNX에서 1,000건 중 44건 비교, model-active 13건, agreement 100%, 오류 0건을 확인했다. 다만 production traffic, 현재 online 상태, 야간 window, live interruption과 multi-worker는 증명하지 않는다 | 완료된 배포와 local 배관 검증을 production-grade DLP로 확대 해석하지 않는다. target Linux에서 authenticated Gateway PII E2E, realistic concurrency/timeout/fallback, 장시간 Shadow/Canary와 rollback evidence를 남김 |
