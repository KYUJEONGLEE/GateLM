...
| DOC-018 | `difficulty-feature-vector.v2`의 complete offline bundle은 생성됐지만 runtime packaging과 승격 evidence가 없다 | [`../routing/difficulty-e5-encoder.md`](../routing/difficulty-e5-encoder.md)의 pinned QInt8 encoder/PCA 64D, fixed 4-head/12D와 calibration-only candidate selection으로 고정한 118D decision head·calibrator가 exact component hash를 가진 offline artifact로 생성됐다. 선택 후 단일 frozen candidate의 untouched Holdout evidence는 존재하지만 per-category safety gate, image/runtime packaging, latency·memory·failure isolation, supported-runtime Python-Go numeric parity와 active runtime contract가 모두 pending임 | 현재 bundle을 active contract나 Gateway runtime으로 승격하지 않음. 모든 gate와 새 promotion Holdout을 통과한 뒤에도 먼저 opt-in shadow 결과와 rollback 준비를 검토하고, exact artifact version·bundle hash·threshold policy에 대한 owner의 명시적 promotion을 요구하며 자동 승격하지 않음 |
