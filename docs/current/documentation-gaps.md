...
| DOC-018 | `difficulty-feature-vector.v2`의 opt-in request shadow는 구현됐지만 product routing 승격 evidence가 없다 | [`../routing/difficulty-e5-encoder.md`](../routing/difficulty-e5-encoder.md)의 pinned QInt8 encoder/PCA 64D, fixed 4-head/12D와 calibration-only candidate selection으로 고정한 118D decision head·calibrator가 exact component hash를 가진 artifact로 생성됐다. Linux amd64 optional image/runtime lock, 시작 시 hash 검증, English/Korean/right-truncation 3건의 Python 0.21.2 ↔ Go/Rust 0.22.0 pooled 384D numeric parity gate와 masking 이후 auto 요청의 non-blocking worker 1개/대기 1개/기본 100ms shadow 경계가 추가됐다. Shadow 결과는 rule routing/modelRef/provider 호출을 변경하지 않고 고정 aggregate metric으로만 소비한다. 다만 기존 Holdout의 `general` per-category safety gate 실패와 request-level latency·memory·timeout 후 native inference 자원 회수 evidence는 여전히 pending이다 | 현재 bundle을 product routing decision으로 승격하지 않음. 새 promotion artifact와 untouched Holdout이 모든 gate를 통과하고 runtime evidence와 rollback 기준이 승인된 뒤 exact artifact version·bundle hash·threshold policy에 대한 owner의 명시적 promotion을 요구하며 자동 승격하지 않음 |
