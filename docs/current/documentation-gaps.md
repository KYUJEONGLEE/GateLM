...
| DOC-018 | `difficulty-feature-vector.v2`의 encoder/projection은 provisional 선택됐지만 완전한 feature bundle과 승격 evidence가 없다 | [`../testing/difficulty-semantic-encoder-selection.md`](../testing/difficulty-semantic-encoder-selection.md)는 immutable `multilingual-e5-small` QInt8와 `P=64` projection을 synthetic tooling-smoke에서 선택하고 4개 semantic head의 고정 12차원에 따른 후보 크기 `42`, `106`, `118`을 기록한다. 다만 semantic head/difficulty head/calibrator artifact, approved human-reviewed dataset, active contract 승인은 아직 pending임 | Approved family-disjoint dataset으로 선택·holdout을 재검증하고 나머지 component manifest와 호환 bundle을 승인하기 전에는 proposal을 active contract나 runtime으로 승격하지 않음 |
