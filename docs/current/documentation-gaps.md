...
| DOC-016 | Hybrid `ComplexityScore`의 single-request immutable artifact가 새 promotion Holdout accuracy gate를 통과하지 못했다 | 기존 Candidate C 118D architecture를 유지한 v3 artifact를 변경하지 않고, owner-approved expansion의 아직 보지 않은 holdout 40 family/400건에서 score-independent SHA-256 rank로 category당 2개 whole family, 총 10 family/100건을 먼저 고정했다. 이전 500건과 family overlap은 0이고 category마다 simple/complex가 10/10이다. 첫 aggregate-only evaluation 결과는 accuracy `0.70`, 전체 `complex -> simple=0`이며 category별 complex-to-simple 비악화는 모두 통과했지만 사전 고정한 accuracy `>=0.91` gate는 실패했다. Rule baseline accuracy는 `0.78`이었다 | 이 Holdout으로 model, PCA, semantic head, calibrator, threshold 또는 subset을 재튜닝·재선택하지 않고 현재 artifact를 product routing으로 승격하지 않음. 후속 model 개선은 새 artifact version과 다시 보지 않은 별도 Holdout을 요구함 |
...
| DOC-018 | `difficulty-feature-vector.v2`의 opt-in request shadow는 구현됐지만 product routing 승격 gate는 실패했다 | Single-request v2 encoder/PCA manifest와 v3 118D artifact의 기존 Linux amd64 native replay는 Python/Go label `100/100`, score tolerance, rule routing/modelRef 불변과 failure isolation을 확인했다. 이후 새 score-independent 100건 promotion Holdout을 소비한 결과는 accuracy `0.70`, `complex -> simple=0`, category별 directional 비악화 통과였으나 accuracy `>=0.91` gate 실패다. Evidence는 [`../testing/difficulty-promotion-holdout-100-result.json`](../testing/difficulty-promotion-holdout-100-result.json)에 aggregate만 보존한다. 2026-07-15 routing owner는 exact artifact/bundle/threshold와 `2 GiB` hard limit, 5분 지속 RSS `>1.25 GiB` 또는 cgroup current `>1.75 GiB`, 즉시 rollback 조건을 [`../testing/difficulty-live-shadow-owner-approval.json`](../testing/difficulty-live-shadow-owner-approval.json)으로 제한된 개발 exact-pair shadow에만 승인했다 | 현재 bundle을 product routing decision으로 승격하지 않음. 개발 exact-pair opt-in도 product decision이 아닌 aggregate disagreement 관찰에만 사용하며, 같은 Holdout으로 재튜닝하지 않음. 승인된 메모리 한도와 rollback 조건을 배포 플랫폼에 적용하기 전에는 live shadow를 활성화하지 않음 |
