...
이 문서는 PR3에서 추가한 Gateway v1 main flow 기준 k6 baseline 실행 결과와 관측 증거를 정리한다. 사용한 API는 기존 계약에 있는 아래 endpoint뿐이다.
...
PR3는 API, DB, Event/Log, Metrics schema를 변경하지 않는다. PR2에서 추가한 `/metrics` endpoint를 소비해서 아래 흐름을 숫자로 확인한다.
...
```powershell
cd C:\Users\KJ\Workspace\gatelm\LLMOps-Gateway\apps\gateway-core
...
```powershell
cd C:\Users\KJ\Workspace\gatelm\LLMOps-Gateway
...
```text
2026-06-27 19:31 Asia/Seoul 로컬 실행 확인 완료.
```
...
```text
range: 2026-06-27T10:31:20Z -> 2026-06-27T10:33:00Z

success:
  requestId=request_d9f7932266a6c044ec1ec2d0a3036d28
  httpStatus=200
  cacheStatus=miss
...
blocked:
  requestId=request_2d240b430a657e39b313301947a10f12
  httpStatus=403
  cacheStatus=bypass
...
Rate limit metric 기록은 PR2 handler test에서 검증했다. 이번 live k6 baseline에서는 rate limit overflow를 강제로 만들지 않는다. 강제로 초과시키면 현재 PostgreSQL fixed-window counter 상태와 환경별 limit 설정에 따라 baseline이 불안정해질 수 있기 때문이다.
...
Prometheus/Grafana scraping 구성은 PR3 범위 밖이다.
