...
Semantic result가 `ready`이면 106D `simple | complex`가 일반 Gateway policy와 Chat App 5×2 policy의 routing matrix cell, ordered modelRef candidate와 decision key의 권위 difficulty가 된다. Non-model-path `not_applicable`과 `unavailable | busy | timeout | invalid_embedding | inference_failed | panic_recovered`는 해당 요청에서 기존 rule difficulty를 유지한다. Category는 항상 rule classifier 결과다. Runtime과 non-authoritative 42D B1 request shadow는 동시에 활성화할 수 없다.
...
Tenant Chat local experiment Compose는 기본 CGO-free [`../../infra/docker/gateway-core.Dockerfile`](../../infra/docker/gateway-core.Dockerfile)을 사용하고 E5 bundle을 준비하거나 image에 포함하지 않는다. Local profile은 `GATEWAY_DIFFICULTY_E5_RUNTIME_ENABLED=false`, 42D B1 request shadow `true`, timeout `100ms`를 고정한다. B0 rule difficulty가 routing authority를 유지하며 B1은 synthetic demo tenant/application exact pair에서 aggregate comparison만 만든다.
