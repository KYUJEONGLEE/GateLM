...
- [`../v2.1.0/training/difficulty-model-path-5000.owner-approved.jsonl`](../v2.1.0/training/difficulty-model-path-5000.owner-approved.jsonl): 기존 승인 데이터의 train/calibration model path 1,880건과 2026-07-16 owner-approved 신규 3,120건을 결합한 실제 Logistic Regression 대상 5,000건이다.
- [`../v2.1.0/training/difficulty-model-path-5000.roles.json`](../v2.1.0/training/difficulty-model-path-5000.roles.json): weight fit 3,000, calibration 1,000, evaluation holdout 750, final promotion holdout 250을 family 단위로 동결한다.
...
실제 training candidate는 모든 포함 record가 `human_review + approved`이고, 독립 family 전체가 한 split에만 있으며, dataset manifest가 `trainingEligible=true`와 versioned minimum-family policy를 선언해야 한다. Checked-in owner-approved 500건/89-family candidate는 이 입력 gate와 train 300/calibration 100/holdout 100 partition을 만족한다. `difficulty-decision-boundary.semantic-empty-combined-8.2026-07-15.v2` 기준으로 이 500건은 train 300/calibration 100/holdout 100 모두 `modelPath=true`다. 기존 single-request v3 artifact는 이전 경계에서 train 244/calibration 85/holdout 64만 model path로 사용해 만든 historical artifact이므로, 새 경계의 모델로 간주하거나 같은 artifact version으로 덮어쓰지 않는다. 이 승인은 offline 학습 입력 자격이며 model coefficient, semantic-head weight, calibrator, holdout 결과, runtime promotion 또는 GA를 승인하지 않는다.
...
주 학습 vector는 실제 category 결과를 사용한다. `expectedCategory`는 category별 집계와 별도 oracle 분석에만 사용한다. Exporter는 hybrid classifier와 같은 precheck로 각 sample에 안전한 boolean `modelPath`와 immutable `decisionBoundaryVersion`을 붙인다. Owner-approved 2,500개를 현재 경계로 합산하면 train 1,405/calibration 475/legacy holdout 477이 model path이고, semantic-empty simple sentinel은 100개, hard-complex sentinel은 43개다. 2026-07-16 owner 승격은 이 중 legacy holdout을 재사용하지 않고 train/calibration model path 1,880건만 선택한 뒤 신규 model-path 3,120건을 더해 정확히 5,000건을 고정한다. Role manifest는 weight fit 3,000, calibration 1,000, evaluation holdout 750, final promotion holdout 250을 분리한다. Logistic Regression weight fit은 Train만, calibrator·threshold 선택은 Calibration만 사용한다. Evaluation holdout은 model 조합 선택에 사용된 뒤 재학습에 넣지 않고, Promotion holdout은 model과 threshold를 고정한 후 최종 승격 근거로 한 번만 사용한다. Empty/meaningless와 hard-complex sentinel 표본은 원래 owner-approved regression dataset에서 end-to-end shadow accuracy와 directional gate로 별도 평가하며 model calibration에 섞지 않는다. Exporter의 vector payload와 `modelPath`는 Python subprocess가 메모리에서 소비하며 fixture, report 또는 log로 저장하지 않는다.
...
이 명령으로 만들어진 기존 single-request v3 artifact는 이전 decision boundary의 train 244/calibration 85/holdout 64 membership을 사용한 historical evidence다. 현재 exporter를 사용한 재학습은 train 300/calibration 100/holdout 100 membership이 되므로, 기존 v3 training policy와 artifact version을 재사용하지 않고 새 decision-boundary-aware policy와 새 artifact identity를 먼저 고정해야 한다. `ruleVectorV1`, pooled/projected embedding, semantic head probability와 sample score는 process-local memory에만 유지한다.
...
현재 selected v3 artifact와 generated Go bundle은 PCA, semantic head, difficulty decision head와 Platt calibrator를 포함하지만 API, DB, Event, Metrics, RuntimeSnapshot, routing policy와 제품 `DifficultyResult` shape를 변경하지 않는다. Gateway shadow 실행 위치와 tokenizer/ONNX image packaging은 별도 optional profile이다. 첫 promotion Holdout은 고정 v3 artifact로 한 번 평가됐고 accuracy `0.70`으로 gate를 실패했으므로 runtime 승격 근거가 되지 않는다.

후속 threshold-only v4 검사는 기존 calibration에서 family-grouped out-of-fold score만 사용했다. Calibration에서는 `difficulty-threshold-v2 = 0.06`이 accuracy `0.95`, `complex -> simple=0`으로 gate를 통과했지만, 소비한 family를 제외하고 새로 동결한 whole-family Holdout에서는 accuracy `0.56`, `complex -> simple=0`, `simple -> complex=44`였다. v4는 실패한 offline artifact로 보존하며 Go codegen, parity replay, live shadow와 product routing으로 승격하지 않는다. 이 두 번째 Holdout도 결과 확인 후에는 재튜닝 입력이 아니다. Aggregate evidence는 [`../testing/difficulty-threshold-v4-evaluation.md`](../testing/difficulty-threshold-v4-evaluation.md)에서 확인하고 정적 봉인 검증은 `corepack pnpm run verify:v2.1-difficulty-threshold-v4`로 실행한다.
