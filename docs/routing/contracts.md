...
Request shadow 활성화에는 process-local `GATEWAY_DIFFICULTY_E5_SHADOW_ENABLED=true`와 non-empty `GATEWAY_DIFFICULTY_E5_SHADOW_ALLOWED_SCOPES`가 모두 필요하다. Current boundary와 다른 historical v3 baseline은 여기에 exact one-time `GATEWAY_DIFFICULTY_E5_SHADOW_BASELINE_WAIVER=difficulty-shadow-baseline-e2e-v3.2026-07-15.v1`도 필요하다. 이 waiver는 고정된 v3 artifact/bundle/content hash와 threshold `0.45`의 E2E 배선 확인에만 적용하며, 다른 artifact identity나 future artifact에는 재사용하지 않는다. Allowlist는 comma-separated `tenantId/applicationId` exact pair이며 whitespace 제거 후 case-sensitive로 비교한다. Wildcard, tenant-only, application-only와 tenant/application 독립 목록의 cross-product는 허용하지 않는다. Empty 또는 malformed allowlist는 전체 shadow를 fail closed로 비활성화하지만 Gateway 자체를 실패시키지 않는다. Scope 판정은 인증과 application scope 검증이 끝난 trusted Gateway identity만 사용하고 client header, body 또는 query의 scope를 신뢰하지 않는다. Tenant/application ID는 eligibility boolean으로 축약한 뒤 router에 전달하며 shadow runner, observer, log와 metric에는 전달하지 않는다. Allowlist 밖 요청은 shadow job이나 shadow metric을 만들지 않으며 `not_applicable`은 allowlist 안의 eligible auto 요청이 semantic input을 만들 수 없을 때만 사용한다.
