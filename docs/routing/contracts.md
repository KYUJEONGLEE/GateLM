...
Gateway 내부 difficulty classifier는 `DifficultyFeatures`에서 deterministic `ComplexityScore`를 계산하고 하나의 global threshold로 `simple | complex`를 확정한다. Score는 provider/model/tier 정보와 결합하지 않으며 외부 routing 계약의 일부가 아니다. API, DB, Event, Metrics, RuntimeSnapshot, routing policy, response, log와 제품 diagnostics에는 score를 추가하지 않는다. 외부 surface에서 score를 사용하려면 별도 contract proposal과 계약 변경이 필요하다.
...
- 내부 `ComplexityScore`는 invocation summary, provider attempt, 비용 정산, cache key, log, metric label 또는 RuntimeSnapshot으로 전달하지 않는다.
...
- difficulty는 내부 `ComplexityScore`와 하나의 global threshold로 결정되고 score 자체는 외부 contract surface에 나타나지 않는다.
