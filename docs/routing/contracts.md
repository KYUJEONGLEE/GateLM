...
### 2.3 Isolated 768D LightGBM offline and limited shadow

기존 106D LR runtime은 계속 authority다. LightGBM 후보는 공통 masking 이후 `instructionText`와 exact 42D rule vector를 재사용하지만 별도 `intfloat/multilingual-e5-base` 768D encoder와 별도 LightGBM head를 사용한다. 허용 E5-base shadow shape는 순수 `768D`, `42+768=810D`, `42+PCA128=170D`, `42+PCA256=298D`이며 PCA는 train split에만 fit한다. 별도 four-way offline 비교는 기존 E5-small 산출물을 읽기 전용으로 재사용하여 `42+PCA64=106D`, `42+semantic-heads12=54D`, `768D`, `42+768=810D`를 같은 split과 학습 설정으로 비교할 수 있다. 이 경로는 [`difficulty-lightgbm-shadow.md`](difficulty-lightgbm-shadow.md)의 offline 평가 또는 제한 shadow 외에는 실행하지 않는다.

LightGBM profile과 authoritative LR profile은 같은 AI Service process에서 동시에 활성화할 수 없다. 별도 process/container, 별도 dependency profile, 별도 service token, 별도 artifact root와 readiness를 사용한다. LightGBM process는 private `POST /internal/routing/difficulty/lightgbm-shadow/v1/classify`만 제공하며 public Gateway API, DB, Event, RuntimeSnapshot 또는 routing policy shape를 변경하지 않는다. Request는 고정 contract/model identity, bounded instruction text와 exact 42D finite vector만 포함하고 response는 고정 identity, `ready`, `simple | complex`만 포함한다.
...
LightGBM `comparison`은 `match | authoritative_simple_shadow_complex | authoritative_complex_shadow_simple | not_compared`로 고정한다. Raw/redacted prompt, instruction/payload text, token, 42D/768D/PCA vector, score, threshold, artifact hash, request/trace/tenant/application ID, modelRef, provider/model과 raw error detail을 외부 response, log, event, DB 또는 metric label에 넣지 않는다.

Runtime profile은 manifest 자체와 모든 tokenizer/encoder/PCA/model artifact의 relative path, size와 SHA-256, model feature count, encoder revision, execution shape와 training provenance를 startup에 검증한다. `promotionState`는 `offline_shadow_only`만 허용한다. 현재 15,000건 candidate dataset은 `training_eligible=false`이고 사람 검수 전이므로 LightGBM 학습·threshold 선택·promotion evidence로 사용할 수 없다. LightGBM의 authority 전환은 별도 active contract와 승인된 evaluation evidence가 필요한 후속 변경이다.
