...
### 2.3 Isolated LightGBM offline and limited shadow

기존 E5-small PCA64 + rule42 Logistic Regression 106D runtime은 계속 authority다. LightGBM 106D는 차원만 같고 model/artifact/version/threshold/runtime identity가 완전히 별개다. LightGBM runtime은 profile에 따라 기존 pinned E5-small encoder/PCA64를 읽기 전용으로 재사용하는 `42+PCA64=106D`와 `42+semantic-heads12=54D`, 또는 별도 pinned E5-base encoder를 사용하는 `768D`와 `42+768=810D`를 실행한다. 기존 E5-base `42+PCA128=170D`, `42+PCA256=298D` profile 호환성도 유지한다. 이 경로는 [`difficulty-lightgbm-shadow.md`](difficulty-lightgbm-shadow.md)의 offline 평가 또는 제한 shadow 외에는 실행하지 않는다.

LightGBM profile과 authoritative LR profile은 같은 AI Service process에서 동시에 활성화할 수 없다. 별도 process/container, 별도 dependency profile, 별도 service token, 별도 artifact root와 readiness를 사용한다. 한 process는 설정된 LightGBM profile 하나만 로드한다. LightGBM process는 private `POST /internal/routing/difficulty/lightgbm-shadow/v1/classify`만 제공하며 public Gateway API, DB, Event, RuntimeSnapshot 또는 routing policy shape를 변경하지 않는다. Request는 고정 contract/model identity, bounded instruction text와 exact 42D finite vector만 포함하고 response는 고정 identity, `ready`, `simple | complex`만 포함한다.

Gateway shadow는 다음 조건을 모두 충족할 때만 요청을 non-blocking queue에 제출한다.

- LR local 또는 remote authoritative runtime이 활성화되어 있음
- global LightGBM shadow enable이 true임
- case-sensitive exact `tenantId/applicationId` pair allowlist에 포함됨
- non-empty request ID의 deterministic hash가 configured `1..10000` basis points sampling에 포함됨
- Gateway configured model version/content hash가 AI Service response와 exact match함

Wildcard, partial match와 tenant/application cross-product는 허용하지 않는다. Queue full, timeout, unavailable, invalid response, model drift, startup failure 또는 shadow panic은 route decision과 provider 실행을 변경하거나 실패시키지 않는다. LightGBM result는 이미 확정된 LR difficulty, routing matrix cell, modelRef, candidate 순서, decision key, cache와 budget에 사용할 수 없다.

허용되는 aggregate metric은 다음 두 family뿐이다.
...
LightGBM `comparison`은 `match | authoritative_simple_shadow_complex | authoritative_complex_shadow_simple | not_compared`로 고정한다. Raw/redacted prompt, instruction/payload text, token, 42D rule vector, 384D/768D embedding, PCA vector, semantic-head probability, score, threshold, artifact hash, request/trace/tenant/application ID, modelRef, provider/model과 raw error detail을 외부 response, log, event, DB 또는 metric label에 넣지 않는다. 중간 feature는 AI Service process memory 밖으로 전달하지 않는다.

Runtime profile은 `encoderMode`, exact feature order, manifest 자체와 모든 tokenizer/encoder/PCA/semantic-head/model artifact의 relative path, size와 SHA-256, encoder revision, pooling, semantic-head version/head/class order, execution shape와 training provenance를 startup에 검증한다. 로드된 LightGBM `booster.num_feature()`는 profile의 `numFeatures`와 `totalDimension`에 모두 exact match해야 한다. `promotionState`는 `offline_shadow_only`만 허용한다. 현재 15,000건 candidate dataset은 `training_eligible=false`이고 사람 검수 전이므로 LightGBM 학습·threshold 선택·promotion evidence로 사용할 수 없다. LightGBM의 authority 전환은 별도 active contract와 승인된 evaluation evidence가 필요한 후속 변경이다.
