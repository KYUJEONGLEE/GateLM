...
Hot-path 활성화는 process-local `GATEWAY_DIFFICULTY_E5_RUNTIME_ENABLED=true`를 사용한다. `GATEWAY_DIFFICULTY_E5_RUNTIME_TIMEOUT_MS`는 bounded request timeout을 설정한다. Runtime과 42D B1 request shadow를 동시에 활성화하면 configuration error로 시작을 거부한다. Checked-in artifact는 current `semantic-empty / combined score-8` boundary와 일치하며 historical baseline waiver를 요구하거나 허용하지 않는다. 기본 CGO-free Gateway image는 E5를 포함하지 않고 runtime을 disabled로 유지한다. 권위 runtime image는 [`../../infra/docker/gateway-core-e5-runtime.Dockerfile`](../../infra/docker/gateway-core-e5-runtime.Dockerfile)이다.

### 2.2 42D B1 request-shadow observability

`GATEWAY_DIFFICULTY_E5_SHADOW_ENABLED`와 exact-pair allowlist 환경변수 이름은 배포 호환성을 위해 유지하지만, shadow classifier는 E5 encoder, embedding, PCA 또는 기존 106D artifact를 사용하지 않는다. Model path 요청은 고정 `difficulty-feature-vector.v1` 42D만 입력으로 사용하고, `difficulty-logistic.semantic-b1.model-path-5000.2026-07-19.v1`의 L2/liblinear Logistic Regression `C=10`, Isotonic calibrator와 전역 `difficulty-threshold.semantic-b1-fixed-0_5.2026-07-19.v1 = 0.5`를 적용한다. Artifact content hash는 `sha256:6fdd46325175cb36189f33c2e590841165be13f081c44b982400da13f17d38a9`이며 `ComplexityScore >= 0.5`이면 `complex`, 미만이면 `simple`이다.

이 B1 shadow는 non-authoritative 비교용 compatibility surface다. Hot-path runtime이 enabled이면 사용할 수 없다. 초기화 시 고정 artifact version/hash, Isotonic과 threshold `0.5`가 일치하지 않으면 shadow를 시작하지 않는다. Shadow 결과는 routing matrix, modelRef, cache 또는 provider 호출에 영향을 주지 않으며 offline 학습을 실행하거나 artifact를 자동 갱신하지 않는다.

Tenant Chat local experiment profile은 106D hot-path runtime을 끄고 B0 rule difficulty를 routing authority로 유지한 채 B1 shadow를 켠다. Local profile은 synthetic demo tenant/application exact pair만 기본 allowlist에 넣으며, 다른 pair는 `GATEWAY_DIFFICULTY_E5_SHADOW_ALLOWED_SCOPES`에 명시적으로 추가되기 전까지 평가하거나 metric을 만들지 않는다. AWS production profile의 별도 106D runtime 설정은 이 local experiment 전환에 포함되지 않는다.
...
- difficulty target contract는 artifact마다 단일 전역 Logistic Regression, 전역 calibrator와 전역 threshold 하나만 허용한다. 현재 optional 106D authoritative model-path runtime은 `0.096`을 사용한다. Non-authoritative request shadow는 별도 42D B1 artifact의 Isotonic과 `0.5`를 사용하며, category와 non-model-path difficulty는 계속 rule-based다.
