...
현재 as-built에는 `difficulty-feature-vector.v1` encoder, [`difficulty-logistic-training.md`](difficulty-logistic-training.md)의 비활성 offline 준비 tooling, validated artifact를 명시적으로 입력받는 hybrid classifier와 opt-in shadow evaluator가 존재한다. Tooling은 500건 synthetic training-tooling smoke 재생성, family-disjoint partition, sentinel과 model path를 구분하는 canonical Go vector export, model-path 표본만 사용하는 Python Logistic Regression·calibrator fit, JSON artifact 검증·Go code generation과 package-private inference math를 제공한다. 원본 500건 smoke는 `human review pending + trainingEligible=false`로 계속 학습에 사용할 수 없다. 별도로 파생된 owner-approved 500건/89-family candidate는 `human_review + approved + trainingEligible=true`와 family-disjoint train 300/calibration 100/holdout 100 partition을 갖춘 offline 학습 입력이다. Train 300건으로 fit한 canonical E5/PCA 64D artifact는 checked in됐지만 model/calibrator artifact와 이 dataset의 holdout evidence는 아직 없다. Shadow evaluator는 `-difficulty-shadow-model-artifact`가 없으면 실행되지 않으며 `SimpleRouter`와 current product runtime은 계속 rule-based classifier를 사용한다. Approved dataset으로 만든 downstream immutable artifact, family-disjoint evidence와 현재 rule-based baseline 대비 safety gate가 준비되기 전까지 runtime behavior를 유지한다.
...
Package-private `difficultyEmbeddingInput`은 offline semantic encoder의 dormant input boundary다. 의미 있는 `instructionText`만 반환하며 `normalizedText`, `payloadText` 또는 raw prompt로 fallback하지 않는다. Payload-only 또는 의미 없는 instruction이면 encoder candidate가 없다고 반환한다. Approved offline tooling은 `DifficultySemanticInputForOffline` bridge로 이 exact boundary만 process-local에서 사용할 수 있으며 반환 text를 report, log, metric, lock manifest 또는 artifact에 저장하지 않는다. Empty input은 tokenizer 이전에 `not_applicable`로 제외하고 zero vector로 교정하지 않는다. Canonical component는 [`difficulty-e5-encoder.md`](difficulty-e5-encoder.md)의 pinned E5 QInt8, attention-mask mean pooling, train-only PCA 384→64와 post-PCA L2를 사용한다. Tokenizer/QInt8는 local artifact cache 또는 향후 Docker image build에 포함하고 runtime download를 금지한다. 비활성 offline 준비 tooling에는 `trainingEligible=true`와 human-approved family를 강제하는 semantic-head exporter, canonical normalized E5/PCA 64D representation에서 고정 4-head/12D linear-softmax candidate를 학습하는 component artifact, aggregate-only 평가와 CLI가 존재한다. Checked-in semantic-head weight artifact, difficulty decision head, calibrator 또는 active runtime bundle은 아직 없다. 이 tooling은 Gateway hot path에서 tokenizer, embedding model, projection, head 또는 classifier를 호출하지 않고 current runtime과 `difficulty-feature-vector.v1` 42차원 계약을 변경하지 않는다.
