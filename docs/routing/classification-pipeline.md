...
| Field | Value |
|---|---|
| Status | Active routing target contract + optional authoritative 106D runtime + non-authoritative 42D B1 shadow |
...
`DifficultyFeatureNamesV1`과 `VectorizeDifficultyFeaturesV1`은 `difficulty-feature-vector.v1`의 고정 42차원 이름·순서·scaling·enum·zero-fill 계약을 구현한다. Vectorizer 결과는 non-authoritative B1 request shadow의 전체 입력이며, optional authoritative E5 runtime에서는 64D PCA projection과 결합되어 106D 입력이 된다. Exact 계약은 [`difficulty-feature-vector-v1.md`](difficulty-feature-vector-v1.md)를 따른다.
...
정확한 feature 순서, 숫자 정규화, bucket/enum encoding과 category zero-fill은 `difficulty-feature-vector.v1`로 고정한다. Coefficient, intercept, regularization/solver 설정, model artifact hash와 calibrator parameter는 별도의 immutable artifact version과 content hash로 고정한다. 현재 request shadow B1은 owner-fixed reset으로 Candidate A 42D, L2/liblinear Logistic Regression `C=10`, Isotonic과 threshold `0.5`를 freeze했다. Train 3,000건과 calibration 1,000건만 사용했고 promotion holdout은 열지 않았다. Artifact version은 `difficulty-logistic.semantic-b1.model-path-5000.2026-07-19.v1`, content hash는 `sha256:6fdd46325175cb36189f33c2e590841165be13f081c44b982400da13f17d38a9`다. Projection `P=6`과 semantic head `C=10`은 이후 semantic 실험의 전역 설정으로 별도 experiment baseline hash에 포함되지만, 이 42D B1 추론 입력이나 content hash에는 포함되지 않는다.
...
초기 bootstrap global threshold policy `difficulty-threshold-v1 = 0.45`는 historical artifact에 그대로 남긴다. Optional authoritative 106D runtime은 `difficulty-threshold.model-path-5000.2026-07-16.v1 = 0.096`을 사용한다. Non-authoritative 42D B1 request shadow는 `difficulty-threshold.semantic-b1-fixed-0_5.2026-07-19.v1 = 0.5`를 사용하며 `ComplexityScore >= 0.5`이면 `complex`다. Request, tenant, RuntimeConfig, RuntimeSnapshot, 환경변수 또는 runtime caller가 어느 threshold도 덮어쓰지 못한다.

Semantic candidate의 pooled E5 input, PCA parameter/output과 post-PCA L2는 canonical `float32`를 사용하고 106D final Logistic Regression, stable sigmoid와 Platt 계산은 `float64`를 사용한다. Semantic heads는 함께 학습·평가한 provenance이지만 선택된 106D input이 아니므로 Gateway 추론 재료와 hot path에 포함하지 않는다. 고정 feature order와 coefficient/intercept를 유지하고 판정 전에 표시용 반올림을 하지 않는다. 같은 versioned artifact와 입력은 지원되는 Go runtime에서 계약된 numeric tolerance와 같은 difficulty를 만들어야 한다. 외부 LLM, network, clock, randomness, runtime 재학습 또는 자동 보정을 inference math에 추가하지 않는다. `NaN`, infinity 또는 `0.0~1.0` 밖 값을 만드는 입력은 semantic result를 non-ready로 처리하고 rule difficulty로 fallback한다. 이 계약은 다른 언어와 CPU 사이의 bit-for-bit 동일성을 약속하지 않는다.
...
현재 as-built에는 `difficulty-feature-vector.v1` vectorizer, [`difficulty-logistic-training.md`](difficulty-logistic-training.md)의 offline tooling, 별도의 106D hot-runtime codegen profile과 42D B1 request-shadow codegen profile이 존재한다. B1 artifact `difficulty-logistic.semantic-b1.model-path-5000.2026-07-19.v1`은 Logistic Regression `C=10`, Isotonic, `0.5`와 content hash를 고정한다. B1 shadow의 Pure Go 경로는 `DifficultyFeatures`에서 42D를 만들어 JSON/file I/O, tokenizer, encoder, embedding과 PCA 없이 평가한다. 기존 106D generated bundle과 E5 tokenizer/ONNX encoder는 optional authoritative hot runtime에만 남으며 B1 shadow에서 생성하거나 호출하지 않는다. Shadow 결과는 `SimpleRouter`의 rule result, RuntimeSnapshot과 product route decision에 연결하지 않는다.
...
Request shadow observer는 `status`, active category와 rule↔42D B1 shadow difficulty의 고정 comparison enum, aggregate duration만 받는다. 개별 `ComplexityScore`를 observer DTO로 전달하지 않으며 허용 metric과 label은 [`contracts.md`](contracts.md)의 42D B1 request shadow 절을 따른다.
...
- E5 runtime artifact가 없거나 runtime이 non-ready이면 현재 fixture와 rule fallback difficulty 결과를 바꾸지 않는다.
- `GATEWAY_DIFFICULTY_E5_RUNTIME_ENABLED=true`인 optional E5 profile에서만 106D result를 authoritative difficulty로 사용한다.
- Runtime과 42D B1 request shadow는 동시에 활성화하지 않는다.
...
- Vectorizer 단독 추가는 authoritative runtime behavior를 변경하지 않는다. 106D hot path에는 exact generated 106D artifact와 explicit runtime enable이 함께 있어야 하고, request shadow에는 exact generated 42D B1 artifact와 explicit shadow enable/allowlist가 함께 있어야 한다.
...
- Optional authoritative runtime artifact는 하나의 전역 Logistic Regression, 전역 Platt calibrator와 전역 `0.096` threshold만 사용한다. Non-authoritative B1 shadow artifact는 별도의 전역 Logistic Regression `C=10`, Isotonic과 전역 `0.5` threshold만 사용한다.
...
- Optional authoritative 106D runtime은 `ComplexityScore >= 0.096`, B1 shadow는 `ComplexityScore >= 0.5`이면 `complex`이고 각각 미만이면 `simple`이다.
...
- Rule fallback은 bounded single-proxy 요청만 `simple` 예외로 처리하고 독립 proxy가 둘 이상이면 기존 complex 판정을 유지한다.
...
- Selected 106D hot-runtime Go bundle과 42D B1 shadow Go artifact는 각각 exact artifact/component/content hash와 regeneration drift gate를 통과해야 한다.
...
- Tokenizer, encoder, embedding과 projection은 기본 CGO-free runtime과 42D B1 shadow에서 실행하지 않는다. 명시적으로 enable한 optional 106D authoritative E5 runtime만 bounded synchronous dispatcher에서 실행하며 semantic head는 어느 Gateway 추론 경로에도 포함하지 않는다.
