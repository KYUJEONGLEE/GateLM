...
Package-private `difficultyEmbeddingInput`은 향후 semantic encoder를 위한 dormant input boundary다. 의미 있는 `instructionText`만 반환하며 `normalizedText`, `payloadText` 또는 raw prompt로 fallback하지 않는다. Payload-only 또는 의미 없는 instruction이면 encoder candidate가 없다고 반환한다. Approved offline tooling은 `DifficultySemanticInputForOffline` bridge로 이 exact boundary만 process-local에서 사용할 수 있으며 반환 text를 report, log, metric, lock manifest 또는 artifact에 저장하지 않는다. Empty semantic representation은 아직 open decision이므로 offline benchmark exporter도 candidate가 없는 sample을 zero vector로 교정하지 않고 fail closed한다. 비활성 offline 준비 tooling에는 `trainingEligible=true`와 human-approved family를 강제하는 semantic-head exporter, frozen pooled encoder representation에서 고정 4-head/12D linear-softmax candidate를 학습하는 component artifact, aggregate-only 평가와 CLI가 존재한다. 현재 approved training dataset과 semantic-head weight artifact는 없으며 이 tooling은 Gateway hot path에서 tokenizer, embedding model, projection, head 또는 classifier를 호출하지 않고 current runtime과 `difficulty-feature-vector.v1` 42차원 계약을 변경하지 않는다.
