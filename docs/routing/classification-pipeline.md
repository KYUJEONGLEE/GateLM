...
`DifficultyFeatureNamesV1`과 `VectorizeDifficultyFeaturesV1`은 `difficulty-feature-vector.v1`의 고정 42차원 이름·순서·scaling·enum·zero-fill 계약을 구현한다. Vectorizer는 model-path 요청에서 64D PCA projection과 결합되어 authoritative 106D runtime 입력이 되며 exact 계약은 [`difficulty-feature-vector-v1.md`](difficulty-feature-vector-v1.md)를 따른다.
...
정확한 feature 순서, 숫자 정규화, bucket/enum encoding과 category zero-fill은 `difficulty-feature-vector.v1`로 고정한다. Coefficient, intercept, regularization/solver 설정, model artifact hash와 calibrator parameter는 별도의 immutable artifact version과 content hash로 고정한다. 현재 shadow selection은 train 3,000건과 calibration 1,000건으로 42D/106D/118D, Platt/Isotonic과 threshold를 비교한 뒤 Candidate B 106D, L2/liblinear `C=10`, Platt, threshold `0.096`으로 freeze했다. 별도 test 1,000건의 joint routing accuracy는 `62.6%`이고 difficulty accuracy는 `97.8%`다. 이 artifact의 존재는 product runtime 등록이나 ML-authoritative 승격을 뜻하지 않는다.
...
초기 bootstrap global threshold policy `difficulty-threshold-v1 = 0.45`는 historical artifact에 그대로 남긴다. 현재 106D shadow는 calibration evidence로 새 immutable `difficulty-threshold.model-path-5000.2026-07-16.v1 = 0.096`을 사용하며 `ComplexityScore >= 0.096`이면 `complex`다. Request, tenant, RuntimeConfig, RuntimeSnapshot, 환경변수 또는 runtime caller가 threshold를 덮어쓰지 못한다.
...
Request shadow observer는 `status`, active category와 rule↔shadow difficulty의 고정 comparison enum, aggregate duration만 받는다. 개별 `ComplexityScore`를 observer DTO로 전달하지 않으며 허용 metric과 label은 [`contracts.md`](contracts.md)의 opt-in 106D request shadow 절을 따른다.
...
- Runtime과 historical request shadow는 동시에 활성화하지 않는다.
...
- Vectorizer 단독 추가는 runtime behavior를 변경하지 않으며, exact generated 106D artifact와 explicit runtime enable이 함께 있어야 한다.
...
- 현재 runtime artifact는 하나의 전역 Logistic Regression, 전역 Platt calibrator와 전역 `0.096` threshold만 사용한다.
...
- `ComplexityScore >= 0.096`이면 `complex`, 미만이면 `simple`이다.
...
- Selected 106D Go bundle은 exact artifact/component/bundle/content hash와 regeneration drift gate를 통과해야 한다.
