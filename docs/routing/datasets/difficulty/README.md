...
| 현재 학습 사용 가능 여부 | 불가 (`training_eligible=false`) |
...
현재 데이터는 사람이 승인한 gold label이 아니다. 합성 데이터는 시나리오 설계 규칙으로, 공개 데이터는 길이를 제외한 고정 규칙과 난이도 점수로 후보 라벨을 부여했다. KLUE는 745개 질문 필드만 사용하며 문맥 직렬화와 KLUE 유래 `rag_query`를 제거했다. 모든 record는 `review_status=pending`, `human_reviewed=false`다. 공개 7,000개 중 사람 원문 기원은 6,848개지만 최종 Prompt 직접 사람 작성 확인분은 3,224개로 60% 목표보다 976개 부족하고, 익명 접근·재배포 가능한 실제 서비스 사용자 Prompt는 0개다. 길이 편향과 유형·도메인 재균형 결과는 [`bias-audit.md`](bias-audit.md)에 기록한다. 이 blocker와 독립 판정·사람 검수가 해소되기 전에는 학습·threshold 선택·runtime 승격 근거로 사용할 수 없다.
...
| OpenAssistant | `prompter` message | Apache-2.0, moderation field 제공 | 1,329개 채택 |
| AI Hub 한국어 데이터 | 한국어 지시·질의 후보 | 로그인·과제별 이용조건으로 익명 재현 불가 | 제외 |
| KoAlpaca | 한국어 instruction | v1.1a 비상업 조건, RealQA 승인형 접근 | 제외 |
| KLUE MRC | 한국어 질문 | CC-BY-SA-4.0, 질문 필드 분리 가능 | 745개 채택 |
| KITE | 한국어 instruction | Apache-2.0, instruction 분리 가능 | 328개 채택 |
| Aya Dataset | 사람 annotation `inputs` | Apache-2.0 | 367개 채택 |
| Dolly 15k | 직원 작성 instruction/context | CC-BY-SA-3.0 | 600개 채택 |
| KULLM-v2 Dolly subset | Dolly의 한국어 번역 instruction/input | Dolly 계보에 CC-BY-SA-3.0 보수 적용 | 2,190개 채택 |
| HRM8K KSM | 한국 수학 question | MIT | 1,199개 채택 |
| K2-Eval | handwritten instruction | MIT | 90개 채택 |
| HAE-RAE BENCH 2.0 | 한국어 benchmark question | MIT | 152개 채택 |
...
최종 구성과 revision별 수량은 [`public-source-review.md`](public-source-review.md)에 기록한다. 가장 큰 source는 KULLM-v2 Dolly subset 2,190개로 공개 component의 31.3%이며 단일 source 40% 상한을 지킨다. exact·정규화 일치와 48-value MinHash LSH 뒤 token unigram/bigram Jaccard 0.90 이상 후보를 제거했고 기존 합성 8,000개와도 비교했다. 이 방식은 embedding 기반 의미 판정의 대체물이 아니므로 사람 검수 단계에서 의미 중복을 다시 판정한다.

KLUE 745개는 `question`만 사용한다. 길이가 인접한 질문끼리 짝지은 뒤 쌍 안에서 의미 난이도를 상대 비교해 라벨 후보를 만들고, KLUE 길이 단독 ROC-AUC 0.55 이하를 검증한다. `context`를 직렬화하지 않으므로 KLUE가 만든 `rag_query`는 0개다. KLUE가 전체 공개분의 대부분이거나 길이·문맥 형식이 난이도 라벨을 직접 누설하던 구조를 제거했지만, 규칙 라벨을 gold 정답으로 만들지는 않으므로 사람 adjudication은 계속 필요하다.
...
|---|---:|---:|---:|---:|
| 한국어 | 80% | 12,000 | 7,400 | 4,600 |
| 영어 | 15% | 2,250 | 200 | 2,050 |
| 한영 혼합 | 5% | 750 | 400 | 350 |
...
KLUE는 질문 단독 745개만 사용한다. 문맥+질문 직렬화와 KLUE 유래 RAG query를 제거했으며 길이가 인접한 질문 쌍 안에서 의미 난이도를 상대 판정한다. 통합 15,000개에서는 긴 Simple/Complex를 각각 860개 확보했고, 각 길이 bucket의 라벨 비중을 35~65%, 길이 단독 ROC-AUC를 0.60 이하로 검증한다. 23개 작업 유형은 424~1,300개 범위다. 엄격 목표와 미충족 항목은 [`bias-audit.md`](bias-audit.md)에 기록한다.
...
의미 중복 검사는 lexical heuristic이므로 승인 단계에서는 multilingual embedding 후보 검색과 사람 판정을 추가한다.
...
- [x] 합성·경계 한국어/영어/혼합 7,400/200/400과 통합 80:15:5
- [x] 모든 언어×라벨 조합
...
- [x] KLUE 800개 상한(현재 745개), 질문 필드만 사용, KLUE 문맥 직렬화·RAG query 0개
- [x] 긴 Simple/Complex 각 860개, bucket별 라벨 35~65%, 길이 단독 ROC-AUC 0.60 이하
- [x] 단일 공개 source 40% 이하와 통합 단일 작업 유형 8.67% 이하
- [x] 사람 원문 기원 공개 Prompt 6,848개(97.8%)
- [ ] 최종 Prompt 직접 사람 작성 4,200개(60%): 현재 3,224개, 976개 부족
- [ ] 익명 접근·재배포 가능한 실제 서비스 사용자 Prompt: 현재 0개
- [ ] 독립 LLM 자동 라벨링과 규칙 검사
- [ ] 사람 검수와 adjudication
- [x] 전체 15,000 통합 JSONL과 manifest 생성
- [ ] training eligibility 승인
- [ ] Champion–Challenger offline evaluation

다음 작업은 직접 사람 작성 Prompt 976개와 승인된 실제 사용자 Prompt를 확보한 뒤 독립 LLM 재판정과 사람 검수를 수행하는 것이다. 공개 7,000개는 규칙 기반 후보 라벨이며 provenance·source·task slice별 평가와 adjudication 전에는 학습에 사용할 수 없다.
