...
- Canonical tokenizer 입력은 `query: ` prefix를 붙이고 special token을 포함하며 maximum 128 token, right truncation을 사용하고 요청 하나만 tokenizer/encoder에 전달한다.
...
|---|---|---|
| Tokenizer | `difficulty-e5-query-prefix-right-truncation-128.single-request.v2` | pinned E5 tokenizer, `query: `, special token, max 128, right truncation, one request per call |
| Encoder | `difficulty-encoder.multilingual-e5-small-qint8.v1` | pinned revision·QInt8 hash, ONNX Runtime CPU, `float32[1,sequence,384]` |
| Pooling | `difficulty-attention-masked-mean.v2` | attention mask `1` token의 mean, padding 제외, `float32[1,384]` |
| Projection | `difficulty-e5-pca-full-svd-64.single-request.v2` | 300 single-request train results, mean `[384]`, components `[64,384]`, post-projection L2 |
...
Encoder/PCA bundle은 [`../../scripts/routing_difficulty_model/artifacts/difficulty-e5-encoder-manifest.v2.json`](../../scripts/routing_difficulty_model/artifacts/difficulty-e5-encoder-manifest.v2.json)에 고정한다. 이 manifest는 `difficulty-e5-single-request-execution.2026-07-15.v1`, batch size `1`, within-request-only padding을 포함하며 적어도 다음 값을 가진다.
...
- Pinned tokenizer·encoder·projection·head·calibrator artifact를 포함하고 시작 시 hash를 검증하는 image/runtime packaging
...
- 전체 및 category별 `complex -> simple` count/rate 비악화 gate. 한 category라도 실패하면 해결 전까지 승격 금지
...
- Supported runtime별 Python-Go numeric parity tolerance, label stability, artifact validation과 deterministic replay
...
위 gate와 active runtime contract 승인이 끝나도 artifact를 production decision path로 자동 전환하지 않는다. 먼저 opt-in shadow로 실행해 invalid result, parity, latency, memory, failure isolation과 current rule-based 대비 safety 결과를 확인한다. Shadow evidence가 승인된 뒤 owner가 exact artifact version·bundle hash·threshold policy와 rollback 기준을 명시적으로 promotion해야만 runtime decision path를 바꿀 수 있다.
