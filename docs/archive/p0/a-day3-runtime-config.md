...
따라서 D는 cache key 생성 시 `requestParamsHash`를 함께 채워야 한다.
...
routingPolicyHash=route_p0_v1
cachePolicyHash=cache_p0_v1
securityPolicyVersionId에는 securityPolicyHash 값을 넣는다.
routingPolicyVersionId에는 routingPolicyHash 값을 넣는다.
cache key version=p0-exact-v2
cache key material에는 selectedProvider/selectedModel, normalizedRedactedPrompt, requestParamsHash가 반드시 들어간다.
```
