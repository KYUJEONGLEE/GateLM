...
## 2026-06-29 4차 반영 - Semantic Cache UI와 Observability 구분

규민님이 제안한 `Exact Cache`와 `Semantic Cache Candidate`의 UI 구분에 동의한다.
Observability에서도 이 둘을 같은 cache hit처럼 집계하면 안 된다.

안전한 해석 기준은 아래다.

| 구분 | Observability 해석 |
| -- | -- |
| Exact Cache Hit | 검증된 v1/v2 main path cache result, provider bypass와 saved cost를 직접 집계 가능 |
| Semantic Cache Candidate | evidence/lab result, similarity와 policy gate 검토 대상 |
| Semantic Cache Bypassed | false hit, safety, policy mismatch, threshold 미달 등으로 미사용된 후보 |
