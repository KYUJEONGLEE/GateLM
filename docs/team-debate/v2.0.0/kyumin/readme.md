...
아래 항목은 공식 v2 계약 전에 팀이 결정해야 한다.

| No | 결정할 것 | 추천안 | 영향 범위 | 우선순위 | 상태 |
| -- | -- | -- | -- | -- | -- |
| 1 | v2 Web Console IA | Dashboard / Management / Analytics / Demo / Settings 분리 | Web, Demo, 발표 동선 | P0 | 미결정 |
| 2 | `teamId`와 `budgetScopeId` UI 표현 | core identity는 유지하고 scope read model로 표현 | Gateway, Control Plane, Web, Observability | P0 | 미결정 |
| 3 | RuntimeSnapshot UI 최소 필드 | version/hash/published metadata 제공 | Control Plane, Gateway, Web, Observability | P0 | 미결정 |
| 4 | Streaming 상태 노출 범위 | v1.x thin slice는 최종 상태, v2는 lifecycle 확장 | Gateway, Web, Observability | P1 | 미결정 |
| 5 | Audience demo 입력 방식 | preset -> controlled input -> limited free input 순서 | Web, Gateway, Safety | P1 | 미결정 |
