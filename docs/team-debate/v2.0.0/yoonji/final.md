...
| RemoteSafetyEngine | v2.0.0 main path 판정권자에서 제외하고 shadow/evaluation path로 제한 |
...
6. RemoteSafetyEngine은 v2.0.0 main path 판정권자에서 제외하고 shadow/evaluation report만 남깁니다.

## 5. v2.0.0까지 남길 것

- v2 safety category 선정 기준
- prompt injection/toxicity/internal keyword의 main path 포함 여부
- RemoteSafetyEngine을 v2.0.0 main path 판정권자에서 제외하는 기준
...
- RemoteSafetyEngine을 v2.0.0 main path 판정권자에서 제외하고 shadow/evaluation으로 제한하는 기준
...
| Remote shadow report | 실험 결과와 Gateway 실제 판단을 구분한다 |
...
- RemoteSafetyEngine shadow result를 Gateway 실제 판단처럼 표시하지 않습니다.
