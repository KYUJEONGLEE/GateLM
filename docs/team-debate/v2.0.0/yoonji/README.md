...
> 아래 내용은 현재 Codex가 병렬 토론 초안으로 보탠 의견입니다. 기존 원격 의견을 대체하지 않으며, Security-sensitive field를 확정하지 않는 working draft입니다.

### 지섭 제안에 대한 보충

- v2.0.0의 safety 목표는 detector 종류를 많이 붙이는 것이 아니라, gateway hot path에서 어떤 판단이 반복 가능하게 일어났는지 설명하는 것이다.
- raw prompt/raw response opt-in은 v2.0.0 기본 목표에서 빼고, redaction/hash/aggregate/synthetic corpus로 evidence를 만든다.
- RemoteSafetyEngine은 shadow/evaluation 축으로 두고, Gateway 필수 runtime dependency가 되기 전에는 fallback/latency/장애 기준을 먼저 정한다.

### 내 역할의 v2.0.0 main path

- PII/secret redaction과 block/allow 판단을 최소 main path로 고정한다.
- prompt injection, toxicity, policy-aware detector는 corpus와 report 기반 shadow evidence로 준비한다.
- false positive/false negative, latency impact, rollback/fallback rule을 발표 evidence에 포함한다.

### 병렬 shadow/evidence 작업

- 합성 corpus로 category별 expected outcome을 만든다.
- raw 원문 없이도 request detail에서 safety decision을 설명할 수 있는 redacted sample을 만든다.
- provider 호출 전 redaction/block이 실제로 선행되는지 smoke test를 구성한다.

### 소비해야 하는 계약 후보

- Gateway의 요청 처리 순서 후보.
- Control Plane의 정책 배포/롤백 snapshot 후보.
- Web Console의 안전한 표시 범위 후보.
- Observability의 safety outcome 집계 후보.

### 생산해야 하는 계약 후보

- Safety decision vocabulary 후보.
- Redaction evidence 형식 후보.
- Synthetic corpus와 evaluation report.
- 발표에서 보여줄 block/redact/allow 시나리오.
