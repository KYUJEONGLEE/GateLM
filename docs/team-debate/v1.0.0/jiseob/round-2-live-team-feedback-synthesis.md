...
## 11. Kyujeong Round 5 반영

Kyujeong의 `round-5-hyeok-round1-response.md`를 마지막으로 읽고 나면, 실제 구현 우선순위는 조금 더 선명해진다.

실제 Provider는 여전히 v1에서 강하게 시도할 가치가 있다. 다만 구현 순서와 데모 안정성 기준에서는 아래 순서가 더 안전하다.
...
즉, 실제 Provider는 "하면 좋은 보너스"가 아니라 v1 candidate 중 가장 강한 축이지만, Customer Demo App과 관측 근거가 없는 상태에서 먼저 붙이면 GateLM의 B2B 메시지보다 Provider 호출 자체가 부각될 수 있다.

따라서 최종 표현은 다음처럼 정리한다.

```text
actual Provider:
  v1 candidate 중 최우선으로 시도한다.
  단, v1 main path의 합격 기준은 mock provider fallback으로 보장한다.
  구현 순서는 demo app, rate limit, metrics/k6, detail/dashboard polish 이후로 둔다.
```

Streaming, Runtime Policy Editor, Custom Regex Rule UI는 v1의 제품성을 높이는 기능이지만, 현재 기준에서는 v2 evidence path로 두는 것이 더 안전하다.
