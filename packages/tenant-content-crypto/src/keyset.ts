...
export function parseWrappingKeySet(value: unknown): WrappingKeySet {
  ...
  try {
    for (const candidate of value.keys) {
      ...
      decodedMaterial.push(integrityKey);
      keys.set(
        candidate.version,
        Object.freeze({ version: candidate.version, wrappingKey, integrityKey }),
      );
    }
