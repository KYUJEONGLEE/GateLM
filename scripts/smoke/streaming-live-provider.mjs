...
const HELP = `GateLM 실제 Provider 스트리밍 확인
...
주의:
  이 스크립트는 수동 live smoke 전용이며 CI에서 실행하지 않습니다.
  실제 Provider 비용이 발생할 수 있습니다.
...
const prompt =
  process.env.GATELM_STREAM_PROMPT?.trim() ||
  "GateLM의 실제 Provider 스트리밍 응답이 어떻게 동작하는지 한국어로 짧게 설명해줘.";
...
try {
  console.log("GateLM 실제 Provider 스트리밍 확인");
  ...
  const response = await fetch(`${gatewayUrl}/v1/chat/completions`, {
    ...
    headers: {
      ...
      "X-GateLM-Feature-Id": "manual_streaming_live_smoke",
    },
    body: JSON.stringify({
      model,
