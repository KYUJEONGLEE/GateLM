...
`specs/gateway/v2.0.0` 아래의 JSON Schema와 fixture는 이 문서를 기준으로 작성한다. Historical implementation plan은 `docs/archive/gateway-v2.0.0-planning/`에 보관한다. API route, DB column, Event field, Metrics label을 추가하거나 변경할 때는 이 문서와 충돌하지 않아야 한다.
...
- `specs/gateway/v2.0.0/schemas/draft/`와 `specs/gateway/v2.0.0/fixtures/draft/`는 기준 위치가 아니다.
...
v2.0.0 구현 전 문서/계약 작업의 historical order는 아래와 같았다. 현재 release readiness 기준에서 implementation plan은 `docs/archive/gateway-v2.0.0-planning/`에 보관한다.

1. P0 legacy field cleanup inventory
2. `docs/archive/gateway-v2.0.0-planning/implementation-plan.md` 작성
3. `docs/archive/gateway-v2.0.0-planning/demo-scenario.md` 작성
