...
describe('WrappingKeyProvider', () => {
  let root: string;

  beforeEach(async () => {
    root = await mkdtemp(join(tmpdir(), 'gatelm-content-keys-'));
  });
  ...
  it('loads an active plus grace reader key set without exposing values', async () => {
    ...
    const provider = new WrappingKeyProvider(config(file));
  ...
  ])('fails closed for malformed or rollback-prone key files', async (value) => {
    const file = join(root, 'keys.json');
...
function config(file: string): ConfigService {
  return { get: (name: string) => (name === 'TENANT_CHAT_CONTENT_KEYS_FILE' ? file : undefined) } as ConfigService;
}
