...
export async function POST(request: Request) {
  ...
  try {
    ...
    const response = NextResponse.json({ continueUrl: '/api/tenant-chat/auth/google/continue' }, { headers: { 'cache-control': 'no-store' } });
    const options = { httpOnly: true, maxAge: 10 * 60, sameSite: 'lax' as const, secure: serverEnv().secureCookies };
    response.cookies.set(COOKIE.oauthState, result.state, { ...options, path: '/auth/google' });
    response.cookies.set(COOKIE.oauthPending, result.authorizationUrl, { ...options, path: '/api/tenant-chat/auth/google' });
    return response;
