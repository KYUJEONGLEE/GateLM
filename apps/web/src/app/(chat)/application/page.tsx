import Link from "next/link";
import { getRequestLocale } from "@/lib/i18n/server-locale";

export default async function ApplicationPage() {
  const locale = await getRequestLocale();
  const text = locale === "ko" ? copy.ko : copy.en;

  return (
    <main className="application-launcher-shell">
      <section className="application-launcher-main" aria-labelledby="application-title">
        <header className="application-launcher-header">
          <div>
            <p>{text.eyebrow}</p>
            <h1 id="application-title">{text.title}</h1>
          </div>
        </header>

        <div className="application-grid" aria-label={text.gridLabel}>
          <Link className="application-card application-card-active" href="/application/chat">
            <span className="application-card-icon" aria-hidden="true">
              C
            </span>
            <span className="application-card-content">
              <strong>{text.chat.title}</strong>
              <small>{text.chat.description}</small>
            </span>
            <span className="application-card-status">{text.open}</span>
          </Link>

          {text.placeholders.map((item) => (
            <div className="application-card" aria-disabled="true" key={item.title}>
              <span className="application-card-icon" aria-hidden="true">
                {item.initial}
              </span>
              <span className="application-card-content">
                <strong>{item.title}</strong>
                <small>{item.description}</small>
              </span>
              <span className="application-card-status application-card-status-muted">
                {text.pending}
              </span>
            </div>
          ))}
        </div>
      </section>
    </main>
  );
}

const copy = {
  en: {
    chat: {
      description: "AI assistant for workspace conversations.",
      title: "Chat"
    },
    eyebrow: "Applications",
    gridLabel: "Available applications",
    open: "Open",
    pending: "Inactive",
    placeholders: [
      {
        description: "Prepared slot for a workplace assistant.",
        initial: "O",
        title: "Ops Assistant"
      },
      {
        description: "Prepared slot for a support workflow.",
        initial: "S",
        title: "Support Desk"
      }
    ],
    title: "Choose an application"
  },
  ko: {
    chat: {
      description: "업무 대화를 위한 AI 어시스턴트",
      title: "Chat"
    },
    eyebrow: "Applications",
    gridLabel: "사용 가능한 애플리케이션",
    open: "열기",
    pending: "비활성",
    placeholders: [
      {
        description: "업무 어시스턴트 연결 자리",
        initial: "O",
        title: "Ops Assistant"
      },
      {
        description: "지원 업무 앱 연결 자리",
        initial: "S",
        title: "Support Desk"
      }
    ],
    title: "애플리케이션 선택"
  }
};
