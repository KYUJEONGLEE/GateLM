...
export async function syncApplicationChatEnvForProjects(
  ...
  if (isEnvFileSyncDisabled()) {
    return;
  }

  const envFilePath = resolveApplicationChatEnvFilePath();
...
function isEnvFileSyncDisabled() {
  const value = process.env.GATELM_APPLICATION_CHAT_ENV_SYNC?.trim().toLowerCase();

  return value === "0" || value === "false" || value === "no" || value === "off";
}

function resolveApplicationChatEnvFilePath() {
  const override = process.env.GATELM_APPLICATION_CHAT_ENV_FILE?.trim();
