...
function normalizeRecentRequests(value: unknown): AiInsightsRecentRequest[] {
  ...
  return value.slice(0, 10).flatMap((item) => {
    ...
    return [{
      ...
      category: normalizeOptionalText(record?.category, 40) ?? undefined,
      costUsd: normalizeNonNegativeNumber(record?.costUsd),
      difficulty: normalizeOptionalText(record?.difficulty, 40) ?? undefined,
      latencyMs: normalizeNonNegativeNumber(record?.latencyMs),
      modelRef: normalizeOptionalText(record?.modelRef, 160) ?? undefined,
      projectName: normalizeOptionalText(record?.projectName, 120) ?? undefined,
      requestId,
      requestedModel: normalizeOptionalText(record?.requestedModel, 160) ?? undefined,
      routingReason: normalizeOptionalText(record?.routingReason, 120) ?? undefined,
