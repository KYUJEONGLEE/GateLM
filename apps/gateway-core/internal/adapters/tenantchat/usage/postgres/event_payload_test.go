...
func TestSettlementEventPayloadEmitsEmptyMaskingDetectedTypesAsArray(t *testing.T) {
	now := time.Date(2026, time.July, 18, 0, 0, 0, 0, time.UTC)
	payload, err := settlementEventPayload(
		"00000000-0000-4000-8000-000000000300",
		"00000000-0000-4000-8000-000000000400",
		...
		tenantchat.RequestContext{
			...
			Safety: &tenantchat.SafetySummary{
				MaskingAction:        "none",
				MaskingDetectedTypes: nil,
				MaskingDetectedCount: 0,
				SafetyPolicyDigest:   "sha256:" + strings.Repeat("A", 43),
			},
		},
		settlementReservation{PricingVersion: 1, CacheOutcome: "miss"},
		tokenPeriod{Start: now, End: now.Add(7 * 24 * time.Hour), Timezone: "Asia/Seoul"},
		"normal",
		"normal",
		[]tenantchat.ProviderAttempt{{AttemptNo: 1, Outcome: "succeeded"}},
	...
	if event.MaskingAction != "none" || event.MaskingDetectedTypes == nil ||
		len(event.MaskingDetectedTypes) != 0 || event.MaskingDetectedCount != 0 {
		t.Fatalf("expected an explicit empty masking observation, got %+v", event)
	}
}
