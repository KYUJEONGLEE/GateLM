...
func TestChatCompletionsSafetyRoutingCacheDemo(t *testing.T) {
	...
	t.Logf("\n[Given]\n?†Ìö®??API Key?Ä App Token???àÍ≥†, active runtime config?êÏÑú rule-based safety, model=auto routing, exact cacheÍ∞Ä ÏºúÏ†∏ ?àÎã§.")
	...
	t.Logf("\n[Then #1 - Ï∂úÎ†•]\n%s", demoSuccessHTTPOutput(t, first, firstResp, map[string]any{
		...
		"?òÎ?":            "Ï≤?safe ?îÏ≤≠?Ä providerÎ•?1???∏Ï∂ú?òÍ≥† exact cache???Ä?•Îêú??",
	}))
	t.Logf("\n[When #2 - ?ÖÎ†•]\n%s", demoHTTPRequest(t, safePrompt))
	t.Logf("\n[Then #2 - Ï∂úÎ†•]\n%s", demoSuccessHTTPOutput(t, second, secondResp, map[string]any{
		...
		"?òÎ?":                "Í∞ôÏ? safe ?îÏ≤≠?Ä exact cache hitÎ°??ëÎãµ?òÎ©∞ provider ÎπÑÏö©???§Ïãú ÎßåÎì§ÏßÄ ?äÎäî??",
	}))
	...
	t.Logf("\n[Then #3 - Ï∂úÎ†•]\n%s", demoSuccessHTTPOutput(t, redacted, redactedResp, map[string]any{
		...
		"?òÎ?":                              "email/phone?Ä Ï∞®Îã®?òÏ? ?äÍ≥† redaction ??providerÎ°??ÑÎã¨?úÎã§.",
	}))
	...
	t.Logf("\n[Then #4 - Ï∂úÎ†•]\n%s", demoErrorHTTPOutput(t, blocked, blockedResp, map[string]any{
		...
		"?òÎ?":                        "credential-like ?ÖÎ†•?Ä 403?ºÎ°ú Ï∞®Îã®?òÍ≥† cache key build/cache lookup/provider call ?ÑÏóê Î©àÏ∂ò??",
	}))
}
