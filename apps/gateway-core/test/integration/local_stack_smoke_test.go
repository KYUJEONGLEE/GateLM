...
func TestGatewayLocalStackSmoke(t *testing.T) {
	...
	t.Logf("\n[?˜ë?]\nGateway ?œë²„ê°€ ?¤ì œ Docker dependency???°ê²°?˜ì–´ ?ˆê³ , Gateway ?œë©´?ì„œ Mock Provider ëª¨ë¸ catalogë¥?ì¡°íšŒ?????ˆìŒ???•ì¸?œë‹¤.")
	...
	t.Logf("\n[Given]\n? íš¨??API Key/App Tokenê³?runtime configê°€ ?ˆê³ , exact cache?ëŠ” ?„ì§ ê°™ì? ?”ì²­ ê²°ê³¼ê°€ ?†ë‹¤.")
	...
	t.Logf("\n[?˜ë?]\n?ˆì „??ì²??”ì²­?€ rate limit, safety, cache miss, routing??ê±°ì³ Providerê¹Œì? ?„ë‹¬?˜ê³  success/missë¡??ë‚œ??")
	...
	t.Logf("\n[Given]\n?™ì¼ Application??ê°™ì? safe requestë¥??¤ì‹œ ë³´ë‚¸??")
	...
	t.Logf("\n[Then - Gateway ì¶œë ¥]\n%s", prettyJSON(t, map[string]any{
		...
		"body.gate_lm": map[string]any{
			...
			"cacheHitRequestIdEvidence":     "Request Detail?ì„œ ?•ì¸",
	...
	t.Logf("\n[?˜ë?]\nRedis exact cache hit?´ë©´ Providerë¥??¤ì‹œ ?¸ì¶œ?˜ì? ?Šê³ , cache hit requestê°€ ?ë³¸ miss requestIdë¥?ì¶”ì ?œë‹¤.")
	...
	t.Logf("\n[Given]\n?”ì²­ prompt??emailê³?phone_numberê°€ ?¬í•¨?˜ì–´ ?ˆì?ë§?v1 ?•ì±…??redaction ??ê³„ì† ì§„í–‰ ê°€?¥í•œ ? í˜•?´ë‹¤.")
	...
	t.Logf("\n[?˜ë?]\nGateway safety stageê°€ raw ë¯¼ê°ê°’ì„ placeholderë¡?ë°”ê¾¼ ??Provider???„ë‹¬?œë‹¤. Provider stats?ë„ raw ê°’ì? ?¨ì? ?ŠëŠ”??")
	...
	t.Logf("\n[Given]\n?”ì²­ prompt??credential-like secret???¬í•¨?˜ì–´ ?ˆê³  v1 ?•ì±…??block ?€?ì´??")
	...
	t.Logf("\n[?˜ë?]\nblock ?€??ë¯¼ê°?•ë³´??cache key ?ì„±ê³?Provider ?¸ì¶œ ?„ì— 403 sensitive_data_blockedë¡?ì¢…ë£Œ?œë‹¤.")
	...
	t.Logf("\n[Given]\n?™ì¼ Application??60ì´?fixed window?ì„œ local smoke limit??ì´ˆê³¼?ˆë‹¤.")
	...
	t.Logf("\n[?˜ë?]\nRate Limit ì´ˆê³¼ ?”ì²­?€ Provider ë¹„ìš©??ë°œìƒ?˜ê¸° ?„ì— 429 rate_limited terminal outcome?¼ë¡œ ?ë‚œ??")
	...
	t.Logf("\n[Then - Log/Detail/Dashboard/Metrics ì¶œë ¥]\n%s", prettyJSON(t, map[string]any{
	...
	t.Logf("\n[?˜ë?]\n?¤ì œ Gateway ?œë²„ê°€ ë§Œë“  terminal outcome??PostgreSQL Request Log/Detail/Dashboard?€ Prometheus-compatible /metricsê¹Œì? ?´ì–´ì§„ë‹¤. ??smoke??v1.0.0 release candidate baseline??Docker local stack?ì„œ ë°˜ë³µ ?¬í˜„?????ˆë‹¤??ì¦ê±°??")
}
