...
func TestGatewayLocalStackSmoke(t *testing.T) {
	...
	t.Logf("\n[Given]\nDocker Postgres, Redis, Mock Provider, ?§Ï†ú cmd/gateway ?úÎ≤ÑÍ∞Ä ÏºúÏ†∏ ?àÍ≥† demo tenant/project/application seedÍ∞Ä ?ÅÏö©?òÏñ¥ ?àÎã§.")
	...
	t.Logf("\n[?òÎ?]\nGateway ?úÎ≤ÑÍ∞Ä ?§Ï†ú Docker dependency???∞Í≤∞?òÏñ¥ ?àÍ≥†, Gateway ?úÎ©¥?êÏÑú Mock Provider Î™®Îç∏ catalogÎ•?Ï°∞Ìöå?????àÏùå???ïÏù∏?úÎã§.")
	...
	t.Logf("\n[Given]\n?†Ìö®??API Key/App TokenÍ≥?runtime configÍ∞Ä ?àÍ≥†, exact cache?êÎäî ?ÑÏßÅ Í∞ôÏ? ?îÏ≤≠ Í≤∞Í≥ºÍ∞Ä ?ÜÎã§.")
	...
	t.Logf("\n[?òÎ?]\n?àÏ†Ñ??Ï≤??îÏ≤≠?Ä rate limit, safety, cache miss, routing??Í±∞Ï≥ê ProviderÍπåÏ? ?ÑÎã¨?òÍ≥† success/missÎ°??ùÎÇú??")
	...
	t.Logf("\n[Given]\n?ôÏùº Application??Í∞ôÏ? safe requestÎ•??§Ïãú Î≥¥ÎÇ∏??")
	...
	t.Logf("\n[Then - Gateway Ï∂úÎ†•]\n%s", prettyJSON(t, map[string]any{
		...
		"body.gate_lm": map[string]any{
			...
			"cacheHitRequestIdEvidence":     "Request Detail?êÏÑú ?ïÏù∏",
	...
	t.Logf("\n[?òÎ?]\nRedis exact cache hit?¥Î©¥ ProviderÎ•??§Ïãú ?∏Ï∂ú?òÏ? ?äÍ≥†, cache hit requestÍ∞Ä ?êÎ≥∏ miss requestIdÎ•?Ï∂îÏ†Å?úÎã§.")
	...
	t.Logf("\n[Given]\n?îÏ≤≠ prompt??emailÍ≥?phone_numberÍ∞Ä ?¨Ìï®?òÏñ¥ ?àÏ?Îß?v1 ?ïÏ±Ö??redaction ??Í≥ÑÏÜç ÏßÑÌñâ Í∞Ä?•Ìïú ?†Ìòï?¥Îã§.")
	...
	t.Logf("\n[?òÎ?]\nGateway safety stageÍ∞Ä raw ÎØºÍ∞êÍ∞íÏùÑ placeholderÎ°?Î∞îÍæº ??Provider???ÑÎã¨?úÎã§. Provider stats?êÎèÑ raw Í∞íÏ? ?®Ï? ?äÎäî??")
	...
	t.Logf("\n[Given]\n?îÏ≤≠ prompt??credential-like secret???¨Ìï®?òÏñ¥ ?àÍ≥† v1 ?ïÏ±Ö??block ?Ä?ÅÏù¥??")
	...
	t.Logf("\n[?òÎ?]\nblock ?Ä??ÎØºÍ∞ê?ïÎ≥¥??cache key ?ùÏÑ±Í≥?Provider ?∏Ï∂ú ?ÑÏóê 403 sensitive_data_blockedÎ°?Ï¢ÖÎ£å?úÎã§.")
	...
	t.Logf("\n[Given]\n?ôÏùº Application??60Ï¥?fixed window?êÏÑú local smoke limit??Ï¥àÍ≥º?àÎã§.")
	...
	t.Logf("\n[?òÎ?]\nRate Limit Ï¥àÍ≥º ?îÏ≤≠?Ä Provider ÎπÑÏö©??Î∞úÏÉù?òÍ∏∞ ?ÑÏóê 429 rate_limited terminal outcome?ºÎ°ú ?ùÎÇú??")
	...
	t.Logf("\n[Then - Log/Detail/Dashboard/Metrics Ï∂úÎ†•]\n%s", prettyJSON(t, map[string]any{
	...
	t.Logf("\n[?òÎ?]\n?§Ï†ú Gateway ?úÎ≤ÑÍ∞Ä ÎßåÎì† terminal outcome??PostgreSQL Request Log/Detail/Dashboard?Ä Prometheus-compatible /metricsÍπåÏ? ?¥Ïñ¥ÏßÑÎã§. ??smoke??v1.0.0 release candidate baseline??Docker local stack?êÏÑú Î∞òÎ≥µ ?¨ÌòÑ?????àÎã§??Ï¶ùÍ±∞??")
}
